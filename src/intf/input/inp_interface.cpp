...
static bool bAtomiswaveResetKeyLatched = false;

static bool InputIsEmbeddedFlycastArcadeDriver()
{
	if (!bDrvOkay) {
		return false;
	}

	char* system = BurnDrvGetTextA(DRV_SYSTEM);
	if (system == NULL) {
		return false;
	}

	return strcmp(system, "Sega Atomiswave") == 0 || strcmp(system, "Sega Naomi") == 0;
}
	...
	for (i = 0, pgi = GameInp; i < nGameInpCount; i++, pgi++) {
		...
		switch (pgi->nInput) {
			...
			case GIT_SWITCH: {						// Digital input
				...
				if (pgi->nIdent == GK_RESET && InputIsEmbeddedFlycastArcadeDriver()) {
					if (s) {
						if (!bAtomiswaveResetKeyLatched) {
							pgi->Input.nVal = 1;
							bAtomiswaveResetKeyLatched = true;
						} else {
							pgi->Input.nVal = 0;
						}
					} else {
						pgi->Input.nVal = 0;
						bAtomiswaveResetKeyLatched = false;
					}
