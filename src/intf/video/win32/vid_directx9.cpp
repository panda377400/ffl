...
static int dx9CreateDeviceWithFallback(DWORD dwBehaviorFlags, D3DPRESENT_PARAMETERS* pd3dpp)
{
	if (SUCCEEDED(pD3D->CreateDevice(nD3DAdapter, D3DDEVTYPE_HAL, hVidWnd, dwBehaviorFlags, pd3dpp, &pD3DDevice))) {
		return 0;
	}

	if (nVidFullscreen && pd3dpp->BackBufferFormat == D3DFMT_R5G6B5) {
...
static int dx9Init()
{
	...
	if (dx9CreateDeviceWithFallback(dwBehaviorFlags, &d3dpp)) {
