...
static INT32 arm7_thumb_stack_align_fix = 0;
...
UINT32 Arm7DbgGetR(INT32 reg)
{
	if (reg < 0 || reg > 15) return 0;
	return ARM7REG(reg);
}

UINT32 Arm7DbgGetCPSR()
{
	return GET_CPSR;
}

void Arm7SetThumbStackAlignFix(INT32 enable)
{
	arm7_thumb_stack_align_fix = enable ? 1 : 0;
}
...
int Arm7Scan(int nAction)
{
	...
	if (nAction & ACB_DRIVER_DATA) {
		...
		SCAN_VAR(arm7_thumb_stack_align_fix);
	}
