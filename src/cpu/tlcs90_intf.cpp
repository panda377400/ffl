...
static UINT8 (*tlcs90_read_handler)(UINT32) = NULL;
static void (*tlcs90_write_handler)(UINT32, UINT8) = NULL;
static UINT8 (*tlcs90_readio_handler)(UINT16) = NULL;
static void (*tlcs90_writeio_handler)(UINT16, UINT8) = NULL;
...
UINT8 tlcs90_program_read_byte(UINT32 address)
{
	...
	if (tlcs90_read_handler) {
		return tlcs90_read_handler(address);
	}
...
void tlcs90_program_write_byte(UINT32 address, UINT8 data)
{
	...
	if (tlcs90_write_handler) {
		tlcs90_write_handler(address, data);
		return;
	}
}
...
//	bprintf (0, _T("Read Port: %4.4x\n"), port);
	...
	if (tlcs90_readio_handler) {
		return tlcs90_readio_handler(port);
	}
...
//	bprintf (0, _T("Write Port: %4.4x %2.2x\n"), port, data);
	...
	if (tlcs90_writeio_handler) {
		return tlcs90_writeio_handler(port, data);
	}
}
...
INT32 tlcs90Init(INT32, INT32 clock)
{
	...
	tlcs90_read_handler = NULL;
	tlcs90_write_handler = NULL;
	tlcs90_readio_handler = NULL;
	tlcs90_writeio_handler = NULL;
