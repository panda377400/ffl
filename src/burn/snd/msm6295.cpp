...
static INT32 MSM6295CompatMode[MAX_MSM6295];
#if defined FBNEO_DEBUG
static INT32 MSM6295DebugLogCount[MAX_MSM6295];
#endif

static inline INT32 MSM6295UseMameSampleLayout(INT32 nChip)
{
	return MSM6295CompatMode[nChip] & 1;
}

static inline INT32 MSM6295UseStrictSampleValidation(INT32 nChip)
{
	return MSM6295CompatMode[nChip] & 2;
}

static UINT8 *pBankPointer[MAX_MSM6295][0x40000/0x100];
INT32 nLastMSM6295Chip;

void MSM6295SetCompatibilityMode(INT32 nChip, INT32 nMode)
{
#if defined FBNEO_DEBUG
	if (!DebugSnd_MSM6295Initted) bprintf(PRINT_ERROR, _T("MSM6295SetCompatibilityMode called without init\n"));
	if (nChip > nLastMSM6295Chip) bprintf(PRINT_ERROR, _T("MSM6295SetCompatibilityMode called with invalid chip number %x\n"), nChip);
#endif

	MSM6295CompatMode[nChip] = nMode;
}
...
void MSM6295Reset(INT32 nChip)
{
	...
	MSM6295CompatMode[nChip] = 0;
#if defined FBNEO_DEBUG
	MSM6295DebugLogCount[nChip] = 0;
#endif
...
void MSM6295Write(INT32 nChip, UINT8 nCommand)
{
	...
	if (MSM6295[nChip].bIsCommand) {
		...
		INT32 nChannel, nSampleStart, nSampleCount, nSampleStop;
		INT32 nVolume = nCommand & 0x0F;
		INT32 nSampleId = (MSM6295[nChip].nSampleInfo >> 3) & 0x7f;
		nCommand >>= 4;
		...
		for (nChannel = 0; nChannel < 4; nChannel++) {
			if (nCommand & (0x01 << nChannel)) {
				if (MSM6295[nChip].ChannelInfo[nChannel].nPlaying == 0) {
					INT32 bValidSample = 1;

					if (MSM6295UseMameSampleLayout(nChip)) {
						nSampleStart  = MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 0) << 16;
						nSampleStart |= MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 1) <<  8;
						nSampleStart |= MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 2) <<  0;

						nSampleStop  = MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 3) << 16;
						nSampleStop |= MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 4) <<  8;
						nSampleStop |= MSM6295ReadData(nChip, (MSM6295[nChip].nSampleInfo & 0x03ff) + 5) <<  0;

						nSampleStart &= 0x3ffff;
						nSampleStop &= 0x3ffff;
						bValidSample = (nSampleStop > nSampleStart);
						nSampleCount = (nSampleStop - nSampleStart + 1) << 1;
						nSampleStart <<= 1;
					} else {
						...
						bValidSample = (nSampleCount > 0);
					}

					MSM6295[nChip].nSampleInfo &= 0xFF;
#if defined FBNEO_DEBUG
					if (MSM6295CompatMode[nChip] && MSM6295DebugLogCount[nChip] < 512) {
						bprintf(PRINT_NORMAL, _T("[msm6295] chip=%d sample=%02x ch=%d vol=%x start=%05x stop=%05x count=%05x valid=%d compat=%d\n"),
							nChip, nSampleId, nChannel, nVolume, nSampleStart >> (MSM6295UseMameSampleLayout(nChip) ? 1 : 0), nSampleStop, nSampleCount, bValidSample, MSM6295CompatMode[nChip]);
						MSM6295DebugLogCount[nChip]++;
					}
#endif
					
					if ((!MSM6295UseStrictSampleValidation(nChip) || bValidSample) && nSampleCount < 0x80000) {
						...
						MSM6295[nChip].ChannelInfo[nChannel].nSample = MSM6295UseMameSampleLayout(nChip) ? 0 : -1;
						MSM6295[nChip].ChannelInfo[nChannel].nDelta = 0;
...
#if defined FBNEO_DEBUG
			if (MSM6295CompatMode[nChip] && MSM6295DebugLogCount[nChip] < 512) {
				bprintf(PRINT_NORMAL, _T("[msm6295] chip=%d cmd=%02x startcmd status=%02x compat=%d\n"), nChip, nCommand, nMSM6295Status[nChip], MSM6295CompatMode[nChip]);
				MSM6295DebugLogCount[nChip]++;
			}
#endif
...
#if defined FBNEO_DEBUG
			if (MSM6295CompatMode[nChip] && MSM6295DebugLogCount[nChip] < 512) {
				bprintf(PRINT_NORMAL, _T("[msm6295] chip=%d stop=%02x status=%02x compat=%d\n"), nChip, nCommand, nMSM6295Status[nChip], MSM6295CompatMode[nChip]);
				MSM6295DebugLogCount[nChip]++;
			}
#endif
			nCommand >>= 3;
