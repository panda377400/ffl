...
void ics2115_sync()
{
	...
	if (!DebugSnd_ICS2115Initted) {
		bprintf(PRINT_ERROR, _T("ics2115_sync called without init\n"));
		return;
	}
#endif

	if (pBurnSoundOut == NULL || nBurnSoundRate <= 0 || BurnTimerCPUClockspeed <= 0) {
		return;
	}

	INT32 pos = get_stream_pos();
	if (pos < 0) pos = 0;
	if (pos > nBurnSoundLen) pos = nBurnSoundLen;
...
void ics2115_set_xa_compat(INT32 enable)
{
	m_xa_compat = (enable != 0);
}
...
static UINT16 ics2115_reg_read()
{
	...
	if (m_xa_compat && ics2115_xa_reg_override(&ret)) {
		return ret;
	}
...
bool ics2115_xa_reg_override(UINT16 *value)
{
    ...
    {
        case 0x00: {
            UINT8 data = voice.osc_conf.value & ~0x08;
            if (voice.osc.ctl == 0) data |= 0x08;
            *value = data << 8;
            return true;
        }

        case 0x0d: {
            UINT8 data = (voice.osc.vmode == 0 && (voice.vol_ctrl.value & 0x20)) ? 0x81 : 0x01;
            *value = data << 8;
            return true;
        }
