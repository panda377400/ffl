...
void pgmSetEnableArm7(INT32 enable)
{
    nEnableArm7 = enable ? 1 : 0;
}

INT32 pgmGetEnableArm7()
{
    return nEnableArm7;
}
...
static INT32 pgmMemIndex()
{
	...
	INT32 armrom_len = (bDoIpsPatch || (0 != nPGMSpriteBufferHack)) ? 0x0008000 : 0x0004000;

	// kovgsyxp / kovzscs2s 使用 LPC2132 内部Flash，ARM7 ROM 为 0x10000
	const char* drv = BurnDrvGetTextA(DRV_NAME);
	if (drv && (!strncmp(drv, "kovgsyxp", strlen("kovgsyxp")) ||
				!strncmp(drv, "kovzscs2s",    strlen("kovzscs2s")))) {
		armrom_len = 0x00010000;
	}
...
static INT32 pgmGetRoms(bool bLoad)
{
	...
	// 需要 samples 从 0x800000 开始的集合：先最小化只加你确认的
	INT32 snd800 =
		(strncmp(name, "kov2", 4) == 0) ||
		(strncmp(name, "kovgsyxp", strlen("kovgsyxp")) == 0) ||
		(strncmp(name, "kovzscs2s",    strlen("kovzscs2s"))    == 0) ||
		(strcmp(name, "kovgsyx") == 0);
	...
	UINT8 *PGMSNDROMLoad = ICSSNDROM + (snd800 ? 0x800000 : 0x400000);
	...
	//	bprintf (0, _T("Loading ROM #%d\n"), i);
		...
		if ((ri.nType & BRF_SND) && (ri.nType & 0x0f) == 5) {
			if (bLoad) {
				bprintf(0, _T("[%s] load SND at +%x len=%x\n"), name, (INT32)(PGMSNDROMLoad - ICSSNDROM), ri.nLen);
...
INT32 pgmInit()
{
	...
	// ---------------------------------------------------------
	// 1) 先扫一遍 ROM，得到各区长度（不装载）
	// ---------------------------------------------------------
	pgmGetRoms(false);

	// ---- build sprmask wrap mask (必须在 nPGMSPRMaskROMLen 已知后) ----
	nPGMSPRMaskMaskLen = 0;
	if (nPGMSPRMaskROMLen > 0) {
		nPGMSPRMaskMaskLen = 1;
		while (nPGMSPRMaskMaskLen < nPGMSPRMaskROMLen) {
			nPGMSPRMaskMaskLen <<= 1;
		}
		nPGMSPRMaskMaskLen -= 1;
	}
	...
	// ---------------------------------------------------------
	// 2) 分配 ROM 区（注意：这里不要调用 expand_colourdata）
	// ---------------------------------------------------------
	...
	// ---- 特定游戏处理：kovgsyxp、kovgsyx、kovzscs2s ----
	{
		const char* shortname = BurnDrvGetTextA(DRV_NAME);
		if (shortname) {
			int apply_rom_ignore = 0;
			
			// 检查是否是目标游戏
			if (!strncmp(shortname, "kovgsyxp", strlen("kovgsyxp")) ||
				!strncmp(shortname, "kovzscs2s",    strlen("kovzscs2s")) ||
				!strcmp(shortname, "kovgsyx")) {
				apply_rom_ignore = 1;
			}
			
			if (apply_rom_ignore && PGMTileROM && nPGMTileROMLen == 0x1180000) {
				// 期望：0x180000(video bios) + 0x1000000(t01) = 0x1180000
				// MAME：t01 后半重复，仅用前 0x800000 => 0x180000 + 0x800000 = 0x980000
				const INT32 first  = 0x180000;
				const INT32 halfsz = 0x800000;
				
				if (!memcmp(PGMTileROM + first, PGMTileROM + first + halfsz, halfsz)) {
					nPGMTileROMLen = 0x980000; // 只改长度，不 realloc，避免指针/布局问题
					//bprintf(0, _T("[%S] tile ROM_IGNORE applied: new tilelen=%x\n"), shortname, nPGMTileROMLen);
				} else {
					//bprintf(0, _T("[%S] tile halves not identical, skip ROM_IGNORE\n"), shortname);
				}
			}
		}
	}
	...
	// ---------------------------------------------------------
	// 4) 展开图形：先 tile，再 sprcol
	//    注意：expand_colourdata 必须在 pgmGetRoms(true) 之后调用
	// ---------------------------------------------------------
	expand_tile_gfx();   // expand graphics (tile)
	expand_colourdata(); // expand sprite colour
...
INT32 pgmFrame()
{
	...
	if (!OldCodeMode) {
		...
		//	CPU_IDLE_SYNCINT(1, Zet); // sync'd on reads and writes and at the end of the frame
			static int arm_once = 0;
			if (!arm_once) { bprintf(0, _T("[PGM] Arm7Run enabled & scheduled\n")); arm_once = 1; }
