...
static void reset_kovgsyxp_asic27a()
{
	reset_kovsh_asic27a();
	const char* drv = BurnDrvGetTextA(DRV_NAME);
	if (drv && (!strcmp(drv, "kovgsyxp") || !strcmp(drv, "kovzscs2s")))
	{
		memset(kovgsyxp_d00000_ram, 0, sizeof(kovgsyxp_d00000_ram));

		m_kov_pll_lock_timer = 0;
		m_kov_pll_enabled    = 0;
		m_kov_pll_config     = 0;
		...
		m_kov_i2c1conset = 0;
		m_kov_i2c1dat    = 0;
		m_kov_i2c1_status = 0x08; // MAME 默认 idle 状态
		m_kov_handshake_done = 0;

		// VIC regs reset (match MAME device_reset)
		vic_irq_status = 0;
		vic_fiq_status = 0;
		vic_raw_intr   = 0;
		vic_int_select = 0;
...
void install_protection_asic27a_kovsh32()
{
	pPgmScanCallback = kovsh_asic27aScan;
	pPgmResetCallback = reset_kovsh_asic27a;

	SekOpen(0);
	SekMapMemory(PGMARMShareRAM, 0x4f0000, 0x4f7fff, MAP_ROM); // This is a 32 byte overlay at 4f0000, it is NOT WRITEABLE! (68k Page size in FBN is 1024 byte)
	...
	Arm7Init(0);
	Arm7Open(0);
	Arm7MapMemory(PGMARMROM, 0x00000000, 0x00007fff, MAP_ROM);
//	encrypted 68k rom is mapped		0x08100000, 0x083fffff
	Arm7MapMemory(PGMARMRAM0,		0x10000000, 0x100003ff, MAP_RAM);
	Arm7MapMemory(PGMARMRAM2,		0x50000000, 0x500003ff, MAP_RAM);
	Arm7SetWriteWordHandler(kovsh_asic27a_arm7_write_word);
...
void install_protection_asic27a_kovshp32()
{
	pPgmScanCallback = kovsh_asic27aScan;

	SekOpen(0);
	SekMapMemory(PGMARMShareRAM, 0x4f0000, 0x4f7fff, MAP_ROM);  // 32KB
	...
	Arm7Init(0);
	Arm7Open(0);
	Arm7MapMemory(PGMARMROM, 0x00000000, 0x00007fff, MAP_ROM);  // 32KB
	Arm7MapMemory(PGMARMRAM0,	0x10000000, 0x100003ff, MAP_RAM);
	Arm7MapMemory(PGMARMRAM2,	0x50000000, 0x500003ff, MAP_RAM);
	Arm7SetWriteWordHandler(kovsh_asic27a_arm7_write_word);
	Arm7SetWriteLongHandler(kovsh_asic27a_arm7_write_long);
...
static inline void kovgsyxp_arm7_sync_run(INT32 cycles)
{
...
static inline void kovgsyxp_make_reply_from_cmd(UINT16 high, UINT16 low, UINT16 &out_hi, UINT16 &out_lo)
{

	out_hi = 0x8800;
	out_lo = 0x0000;

	const UINT8 key = high >> 8;
	UINT8 cmd = (high & 0xff) ^ key;
	const UINT16 hack_data = low ^ key ^ (key << 8);

	switch (cmd)
	{
		case 0xb5:	// verified on hardware
		{
			static const UINT8 s[16] = { 'W','D','F','*','Z','S','C','S','-','0','4','5','9',0x16,0,0 };
			out_lo = (UINT16)s[hack_data & 0x0f];
		}
		break;

		case 0xbc:	// verified on hardware
		{
			static const UINT8 s[16] = { 'V','3','0','0','C','N',0xf3,0,0,0,0,0,0,0,0,0 };
			out_lo = (UINT16)s[hack_data & 0x0f];
		}
		break;
		...
		{
			static const UINT8 s[16] = { '2','0','1','1','-','0','4','-','2','8',0xb7,0,0,0,0,0 };
			out_lo = (UINT16)s[hack_data & 0x0f];
		}
		break;
		...
		default:
			// 其他命令先不给“假数据”，避免误导逻辑
			// 后续你从 MAME 把完整 command 表搬过来，就是在这里继续加 case
		break;
	}
}

void __fastcall kovgsyxp_asic27a_write_word(UINT32 address, UINT16 data)
{
	UINT32 offset = (address - 0xd80000) >> 1;
	kovgsyxp_arm7_sync_run(50);

	if (offset & 1)
	{
		m_kov_highlatch_68k_w = data;
		m_kov_lowlatch_arm_w = 0;
		vic_set_irq(16, 1);
		kovgsyxp_arm7_sync_run(200);
	}
	else
	{
		m_kov_lowlatch_68k_w = data;
		m_kov_highlatch_arm_w = 0;
		kovgsyxp_arm7_sync_run(50);
	}
}

UINT16 __fastcall kovgsyxp_asic27a_read_word(UINT32 address)
{
	UINT32 offset = (address - 0xd80000) >> 1;
	kovgsyxp_arm7_sync_run(80);

	UINT16 ret = (offset & 1) ? m_kov_highlatch_arm_w : m_kov_lowlatch_arm_w;
	return ret;
}

void install_protection_asic27a_kovgsyxp()
{
	kov_arm7_initted = 0;
	pPgmResetCallback = reset_kovgsyxp_asic27a;

	SekOpen(0);

	memset(kovgsyxp_d00000_ram, 0, sizeof(kovgsyxp_d00000_ram));
	SekMapMemory(kovgsyxp_d00000_ram, 0xd00000, 0xd003ff, MAP_RAM);

	if (PGM68KROM && nPGM68KROMLen >= 0x400000) {
		UINT8 *last1m = PGM68KROM + (nPGM68KROMLen - 0x100000); // 更通用：取最后 1MB
		SekMapMemory(last1m, 0x500000, 0x5fffff, MAP_ROM);
		SekMapMemory(last1m, 0x600000, 0x6fffff, MAP_ROM);
		SekMapMemory(last1m, 0xf00000, 0xffffff, MAP_ROM);
	}

	SekMapHandler(4, 0xd80000, 0xd80005, MAP_READ | MAP_WRITE);
	SekSetReadWordHandler(4, kovgsyxp_asic27a_read_word);
	SekSetWriteWordHandler(4, kovgsyxp_asic27a_write_word);

	SekClose();

	if (kov_arm7_cpu_initted) {
		Arm7Exit();
		kov_arm7_cpu_initted = 0;
	}

	Arm7Init(0);
	kov_arm7_cpu_initted = 1;

	Arm7Open(0);

	Arm7MapMemory(PGMARMROM,      0x00000000, 0x0000ffff, MAP_ROM);
	Arm7MapMemory(PGMARMShareRAM, 0x40000000, 0x40003fff, MAP_RAM);

	Arm7SetReadByteHandler(kov_arm7_read_byte);
	Arm7SetWriteByteHandler(kov_arm7_write_byte);
	Arm7SetReadWordHandler(kov_arm7_read_word);
	Arm7SetWriteWordHandler(kov_arm7_write_word);
	Arm7SetReadLongHandler(kov_arm7_read_long);
	Arm7SetWriteLongHandler(kov_arm7_write_long);

	Arm7Close();

	m_kov_highlatch_arm_w = 0;
	m_kov_lowlatch_arm_w  = 0;
	m_kov_highlatch_68k_w = 0;
	m_kov_lowlatch_68k_w  = 0;
	...
	m_kov_i2c1conset  = 0;
	m_kov_i2c1dat     = 0;
	m_kov_i2c1_status = 0x08;

	m_kov_handshake_done = 0;

	vic_irq_status = vic_fiq_status = 0;
	vic_raw_intr = vic_int_select = vic_int_enable = 0;
	vic_soft_int = vic_protection = 0;
	vic_vect_addr_cur = vic_def_vect_addr = 0;
	memset(vic_vect_addr, 0, sizeof(vic_vect_addr));
	memset(vic_vect_cntl, 0, sizeof(vic_vect_cntl));
...
static const UINT8 B0TABLE[8] = { 2, 0, 1, 4, 3, 7, 5, 6 }; // Maps char portraits to tables (verified on real hardware)

static const UINT8 BATABLE[0x33] = { // verified on real hardware
	0x00,0x29,0x2c,0x35,0x3a,0x41,0x4a,0x4e,0x57,0x5e,0x77,0x79,0x7a,0x7b,0x7c,0x7d,
	0x7e,0x7f,0x82,0x83,0x84,0x85,0x86,0x87,0x88,0x89,0x8a,0x8b,0x8c,0x8d,0x8e,0x90,
...
//-------------------------------
// ASIC27A Init for kovassgs (V315CN bootleg)

static void __fastcall kovassgs_asic27a_write_word(UINT32 address, UINT16 data)
{
	switch (address & 6)
	{
		case 0:
			lowlatch_to_arm = data; // 68K -> ARM7 low latch
			return;
...
static UINT16 __fastcall kovassgs_asic27a_read_word(UINT32 address)
{
	switch (address & 6)
	{
		case 0:
			pgm_cpu_sync(); // 确保 ARM7 先跑
			return lowlatch_to_68k;
...
void install_protection_asic27a_kovassgs()
{
	pPgmScanCallback = kovsh_asic27aScan; // 通用扫描

	SekOpen(0);

	// 映射共享 RAM，V315CN bootleg 对应 32KB
	SekMapMemory(PGMARMShareRAM, 0x4f0000, 0x4f7fff, MAP_ROM);

	SekMapHandler(4, 0x500000, 0x600005, MAP_READ | MAP_WRITE);
	SekSetReadWordHandler(4, kovassgs_asic27a_read_word);
	SekSetWriteWordHandler(4, kovassgs_asic27a_write_word);
	...
	Arm7Init(0);
	Arm7Open(0);
	Arm7MapMemory(PGMARMROM, 0x00000000, 0x00004D80, MAP_ROM);  // 0x8000 ROM
	Arm7MapMemory(PGMARMRAM0, 0x10000000, 0x100003ff, MAP_RAM);
	Arm7MapMemory(PGMARMRAM2, 0x50000000, 0x500003ff, MAP_RAM);
	...
	// ARM7 idle loop 对应 V315CN
	Arm7SetIdleLoopAddress(0x00000260);

	// 初始化 latch
	highlatch_to_arm = lowlatch_to_arm = 0;
	highlatch_to_68k = lowlatch_to_68k = 0;
}
