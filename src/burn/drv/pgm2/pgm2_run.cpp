...
static INT32  Pgm2PerfDiagEnabled = 0;
...
INT32 pgm2Init()
{
    ...
    Pgm2PerfDiagEnabled = (strcmp(BurnDrvGetTextA(DRV_NAME), "kof98umh") == 0);
    if (Pgm2PerfDiagEnabled) {
        bprintf(PRINT_IMPORTANT, _T("[PGM2PERF] enabled for %hs\n"), BurnDrvGetTextA(DRV_NAME));
    }
...
INT32 pgm2Frame()
{
    ...
    INT32 cpuCyclesThisFrame = Arm9TotalCycles();
    ...
    if (Pgm2PerfDiagEnabled) {
        bool logPerf = (Pgm2RtcFrameCounter <= 10) || ((Pgm2RtcFrameCounter % 30) == 0);
        if (logPerf) {
            bprintf(PRINT_IMPORTANT,
                _T("[PGM2PERF] frame=%d cycles=%d mode=%08X %dx%d sprites=%d zoom=%d flip=%d rev=%d max=%dx%d spr_write=%d spr_blit=%d bg=%d fg=%d\n"),
                Pgm2RtcFrameCounter,
                cpuCyclesThisFrame,
                Pgm2PerfDiag.videoMode,
                Pgm2PerfDiag.screenW, Pgm2PerfDiag.screenH,
                Pgm2PerfDiag.spriteCount,
                Pgm2PerfDiag.zoomedSpriteCount,
                Pgm2PerfDiag.flippedSpriteCount,
                Pgm2PerfDiag.reverseSpriteCount,
                Pgm2PerfDiag.maxSpriteW,
                Pgm2PerfDiag.maxSpriteH,
                Pgm2PerfDiag.spritePixelsWritten,
                Pgm2PerfDiag.spritePixelsBlitted,
                Pgm2PerfDiag.bgPixelsDrawn,
                Pgm2PerfDiag.fgPixelsDrawn);
        }
    }
