...
Pgm2PerfDiagStats Pgm2PerfDiag;
...
static void pgm2_draw_sprites()
{
	...
	for (INT32 di = 0; di < eol_dword - 2; di += 4)
	{
		...
		if (flipx) Pgm2PerfDiag.flippedSpriteCount++;
		if (reverse) Pgm2PerfDiag.reverseSpriteCount++;
		if (sizex > Pgm2PerfDiag.maxSpriteW) Pgm2PerfDiag.maxSpriteW = sizex;
		if (sizey > Pgm2PerfDiag.maxSpriteH) Pgm2PerfDiag.maxSpriteH = sizey;
		...
		if (xrepeats || yrepeats || zx_bits != 0xFFFFFFFFu || zy_bits != 0xFFFFFFFFu) {
...
INT32 pgm2DoDraw()
{
	...
	Pgm2PerfDiag.screenW = w;
	Pgm2PerfDiag.screenH = h;
