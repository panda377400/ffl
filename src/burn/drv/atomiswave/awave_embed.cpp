...
#define retro_keyboard_event flycast_retro_keyboard_event
...
#define retro_load_game flycast_retro_load_game_impl
#define retro_load_game_special flycast_retro_load_game_special
#define retro_unload_game flycast_retro_unload_game_impl
...
#define retro_get_system_directory flycast_retro_get_system_directory
...
#include "flycast/core/libretro/libretro.cpp"

#undef retro_load_game
#undef retro_unload_game

static void flycast_reset_content_paths()
{
	disk_paths.clear();
	disk_labels.clear();
	...
	disk_index = 0;
	disc_tray_open = false;
}

extern "C" bool flycast_retro_load_game(const struct retro_game_info* game)
{
	flycast_reset_content_paths();
	return flycast_retro_load_game_impl(game);
}

extern "C" void flycast_retro_unload_game(void)
{
	flycast_retro_unload_game_impl();
	flycast_reset_content_paths();
}
