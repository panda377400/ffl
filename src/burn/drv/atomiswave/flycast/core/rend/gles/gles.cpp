...
// Flag to track if framebuffer has been cleared (to avoid flickering)
bool fb_cleared_flag = false;

void rend_reset_fb_clear_flag()
{
	fb_cleared_flag = false;
}
...
static bool RenderFrame(void)
{
	...
	{
		// Don't clear the framebuffer on every frame - this causes flickering when
		// the frontend reads the FBO before the final frame is rendered.
		// Only clear on the first framebuffer-only render to avoid garbage data.
		if (!fb_cleared_flag) {
			glcache.ClearColor(0.f, 0.f, 0.f, 0.f);
			glClear(GL_COLOR_BUFFER_BIT);
			fb_cleared_flag = true;
		}
		glBindBuffer(GL_ARRAY_BUFFER, gl.vbo.geometry); glCheck();
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, gl.vbo.idxs); glCheck();
		DrawFramebuffer();
...
struct glesrend : Renderer
{
	...
	void Term() override
   {
	   ...
	   // Reset the framebuffer clear flag so it will be cleared on next game load
