...
static bool lr_environment_cb(unsigned cmd, void* data)
{
	...
	case RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY:
		*(const char**)data = g_save_dir.c_str();
		return true;
/*
	 * Some libretro.h versions define RETRO_ENVIRONMENT_GET_CONTENT_DIRECTORY
	 * and RETRO_ENVIRONMENT_GET_CORE_ASSETS_DIRECTORY to the same command value.
	 * Keep only one switch case, otherwise MinGW reports "duplicate case value".
	 */
	...
	case RETRO_ENVIRONMENT_GET_LIBRETRO_PATH:
		*(const char**)data = "fbneo_flycast_static";
		return true;
#endif
...
{
	if (g_loaded) flycast_retro_reset();
}
