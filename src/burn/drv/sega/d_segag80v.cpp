...
static struct BurnDIPInfo Elim2DIPList[]=
{
	...
	{0x00, 0x01, 0x30, 0x10, "Medium"					},
	{0x00, 0x01, 0x30, 0x20, "Hard"						},
	{0x00, 0x01, 0x30, 0x30, "Hardest"					},

	{0   , 0xfe, 0   ,    4, "Bonus Life"				},
	{0x00, 0x01, 0xc0, 0xc0, "None"						},
	{0x00, 0x01, 0xc0, 0x80, "10000"					},
	{0x00, 0x01, 0xc0, 0x40, "20000"					},
	{0x00, 0x01, 0xc0, 0x00, "30000"					},
...
static struct BurnDIPInfo Elim4DIPList[]=
{
	...
	{0x01, 0x01, 0xc0, 0x80, "10000"					},
	{0x01, 0x01, 0xc0, 0x40, "20000"					},
	{0x01, 0x01, 0xc0, 0x00, "30000"					},

	{0   , 0xfe, 0   ,    8, "Coin A"					},
	{0x01, 0x01, 0xe0, 0xe0, "8 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0xc0, "7 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0xa0, "6 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0x80, "5 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0x60, "4 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0x40, "3 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0x20, "2 Coins 1 Credits"		},
	{0x01, 0x01, 0xe0, 0x00, "1 Coin  1 Credits"		},
...
static struct BurnDIPInfo ZektorDIPList[]=
{
	...
	{0   , 0xfe, 0   ,    4, "Bonus Life"				},
	{0x00, 0x01, 0xc0, 0x00, "None"						},
	{0x00, 0x01, 0xc0, 0xc0, "10000"					},
	{0x00, 0x01, 0xc0, 0x80, "20000"					},
	{0x00, 0x01, 0xc0, 0x40, "30000"					},
...
static struct BurnDIPInfo TacscanDIPList[]=
{
	...
	{0x00, 0x01, 0x0c, 0x0c, "8"						},

	{0   , 0xfe, 0   ,    4, "Difficulty"				},
	{0x00, 0x01, 0x30, 0x00, "Easy"						},
	{0x00, 0x01, 0x30, 0x10, "Normal"					},
	{0x00, 0x01, 0x30, 0x20, "Hard"						},
...
static struct BurnDIPInfo StartrekDIPList[]=
{
	...
	{0x00, 0x01, 0x30, 0x10, "Medium"					},
	{0x00, 0x01, 0x30, 0x20, "Hard"						},
	{0x00, 0x01, 0x30, 0x30, "Tournament"				},
...
static UINT8 spinner_input_read()
{
	...
	INT8 delta = BurnTrackballRead(0);
...
static INT32 res_check()
{
	...
	return 0;
#endif
	bool is_flipped = BurnDrvGetFlags() & BDF_ORIENTATION_FLIPPED;
	...
	if (DrvDips[2] & 1) {
		...
		if (Height != 1080) {
			if (is_flipped) {
				vector_rescale(1080, (1080*800/600));
			} else {
...
static INT32 DrvFrame()
{
...

	{
		...
		if (dialmode == 1) { // zektor, tacscan, startrek
			BurnTrackballReadReset();
			BurnTrackballConfig(0, AXIS_NORMAL, AXIS_NORMAL);
			BurnTrackballFrame(0, Analog[0]*2, 0, 0x01, 0x3f);
			BurnTrackballUDLR(0, 0, 0, DrvJoy4[0], DrvJoy4[1], 0x8);
...
static void spacfury_sh1_write(UINT8 data)
{
	...
	if (data & 0x04)
	{
		...
		BurnSampleStop(4, true);

	if (data & 0x01) if (!BurnSampleGetStatus(0)) BurnSamplePlay(0);
	if (data & 0x40) BurnSamplePlay(8);
	if (data & 0x80) if (!BurnSampleGetStatus(9)) BurnSamplePlay(9);
}
...
{
	if (data & 0x0f) {
		if (BurnSampleGetStatus(6) == SAMPLE_STOPPED) {
			BurnSamplePlay(6);
		}
	} else {
		BurnSampleStop(6, true); // soft-stop
	}
