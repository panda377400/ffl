...
#ifdef PGM2_ONLY
#include "driverlist_pgm2.h"
#elif defined(GB_ONLY)
#include "driverlist_gb.h"
#elif defined(MGBA_ONLY)
#include "driverlist_mgba.h"
#else
...
extern "C" INT32 BurnDrvGetIndex(char* szName)
{
	...
	const char* resolvedName = szName;
	if (0 == strcmp(resolvedName, "mgba_gba")) resolvedName = "gba_gba";
	else if (0 == strcmp(resolvedName, "mgba_gb")) resolvedName = "gba_gb";
	else if (0 == strcmp(resolvedName, "mgba_gbc")) resolvedName = "gba_gbc";

	for (UINT32 i = 0; i < nBurnDrvCount; i++) {
		const char* driverName = pDriver[i]->szShortName;
		if (0 == strcmp(driverName, "mgba_gba")) driverName = "gba_gba";
		else if (0 == strcmp(driverName, "mgba_gb")) driverName = "gba_gb";
		else if (0 == strcmp(driverName, "mgba_gbc")) driverName = "gba_gbc";

		if (0 == strcmp(resolvedName, driverName)) {
//			nBurnDrvActive = i;
			return i;
		}
	}
