...
tIniStruct gamehw_cfg[] = {
	...
	{_T("IGS hardware"),				_T("config/presets/igs.ini"),		true,	{ HARDWARE_IGS, 0 },				"\t\t\t<item list=\"igs\" name=\""			},
	{_T("Game Boy Advance hardware"),	_T("config/presets/gba.ini"),		true,	{ HARDWARE_GBA, 0 },				"\t\t\t<item list=\"gba\" name=\""			},
	{_T("Game Boy hardware"),			_T("config/presets/gb.ini"),		true,	{ HARDWARE_GB, 0 },					"\t\t\t<item list=\"gameboy\" name=\""		},
	{_T("Game Boy Color hardware"),		_T("config/presets/gbc.ini"),		true,	{ HARDWARE_GBC, 0 },				"\t\t\t<item list=\"gbcolor\" name=\""		},
	{_T("PGM hardware"),				_T("config/presets/pgm.ini"),		false,	{ HARDWARE_IGS_PGM, 0 },			"\t\t\t<system name=\""        				},
	{_T("PGM2 hardware"),				_T("config/presets/pgm.ini"),		false,	{ HARDWARE_IGS_PGM2, 0 },			"\t\t\t<system name=\""        				},
...
static tIniStruct gamehw_atomiswave_cfg = {
	_T("Atomiswave hardware"), _T("config/presets/atomiswave.ini"), false, { 0 }, "\t\t\t<system name=\""
};

static tIniStruct gamehw_naomi_cfg = {
	_T("Naomi hardware"), _T("config/presets/naomi1.ini"), false, { 0 }, "\t\t\t<system name=\""
};
...
const tIniStruct* GameInputGetHardwarePresetDesc()
{
	const TCHAR* pszSystem = BurnDrvGetText(DRV_SYSTEM);
	UINT32 nHardwareFlag = GameInputGetHWFlag();

	if (pszSystem) {
		if (!_tcscmp(pszSystem, _T("Sega Atomiswave"))) {
			return &gamehw_atomiswave_cfg;
		}

		if (!_tcscmp(pszSystem, _T("Sega Naomi"))) {
			return &gamehw_naomi_cfg;
		}
	}
...
INT32 ConfigGameLoadHardwareDefaults()
{
	...
	const tIniStruct* pPreset = GameInputGetHardwarePresetDesc();

	if (pPreset) {
#if defined(BUILD_SDL2) && !defined(SDL_WINDOWS)
		_stprintf(szFileName, _T("%s%s"), szFolderName, pPreset->ini);
#else
		_tcscpy(szFileName, pPreset->ini);
#endif
