...
int StatedAuto(int bSave)
{
	...
	TCHAR szStorageName[MAX_PATH] = _T("");
	int nRet;

	RomDataGetStorageName(szStorageName, _countof(szStorageName));

	if (NeoCDInfo_ID() && bDrvSaveAll != 0) {
		_stprintf(szName, _T("%sngcd_%s.fs"), szAppEEPROMPath, NeoCDInfo_Text(DRV_NAME));
	} else {
		_stprintf(szName, _T("%s%s.fs"), szAppEEPROMPath, szStorageName);
	}
...
static void CreateStateName(int nSlot)
{
	...
	if (NeoCDInfo_ID()) {
		...
		_stprintf(szChoice, _T("./savestates/%s slot %02x.fs"), szStorageName, nSlot);
	}
}
	...
	if (nSlot) {
		...
		if (bDrvOkay) {
			...
			if (NeoCDInfo_ID()) {
				...
				_stprintf(szChoice, _T("%s*.fs"), szStorageName);
			}
		} else {
...
int StatedSave(int nSlot)
{
	...
	if (nSlot) {
		...
		if (NeoCDInfo_ID()) {
			...
			_stprintf(szChoice, _T("%s"), szStorageName);
		}
		MakeOfn(szFilter);
