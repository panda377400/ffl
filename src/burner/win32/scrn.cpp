...
static bool IsRomDataHistoryEntry(const TCHAR* pszEntry)
{
	if (pszEntry == NULL || pszEntry[0] == _T('\0')) {
		return false;
	}

	const TCHAR* pszExt = _tcsrchr(pszEntry, _T('.'));
	if (pszExt == NULL || 0 != _tcsicmp(pszExt, _T(".dat"))) {
		return false;
	}

	DWORD dwAttrib = GetFileAttributes(pszEntry);
	return (dwAttrib != INVALID_FILE_ATTRIBUTES && !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY));
}

static void GetCurrentHistoryEntry(TCHAR* pszOut, INT32 nCount)
{
	if (pszOut == NULL || nCount <= 0) {
		return;
	}

	pszOut[0] = _T('\0');

	if (RomDataGetActiveDatPath(pszOut, nCount)) {
		return;
	}

	_tcsncpy(pszOut, BurnDrvGetText(DRV_NAME), nCount - 1);
...
static void UpdatePreviousGameList()
{
	...
	TCHAR szCurrentEntry[MAX_PATH] = { 0 };
	GetCurrentHistoryEntry(szCurrentEntry, _countof(szCurrentEntry));
	if (szCurrentEntry[0] == _T('\0')) {
		return;
	}

	// check if this game is identical to any of the listed in the recent menu
	for (INT32 x = 0; x < SHOW_PREV_GAMES; x++) {
		if(!_tcsicmp(szCurrentEntry, szPrevGames[x])) {
			nRecentIdenticalTo = x;
		}
	}

	// create unshuffled (temp) list
	TCHAR szTmp[SHOW_PREV_GAMES][MAX_PATH];
	for (INT32 x = 0; x < SHOW_PREV_GAMES; x++) {
	...
	switch (nRecentIdenticalTo) {
		case -1:
			...
			_tcscpy(szPrevGames[0], szCurrentEntry);
			break;
		default:
...
int BurnerLoadDriver(TCHAR *pszDriverName)
{
	...
	if (IsRomDataHistoryEntry(pszDriverName)) {
		return RomDataLoadDriver(pszDriverName);
	}
...
static void OnCommand(HWND /*hDlg*/, int id, HWND /*hwndCtl*/, UINT codeNotify)
{
	...
	switch (id) {
...
#endif
				QuitGame();
				nDialogSelect = nOldDlgSelected = nGame;
				if (bDoIpsPatch) {
		...
		case MENU_MEMCARD_CREATE:
			if (bDrvOkay && UseDialogs() && !kNetGame && DriverSupportsMemcard()) {
				InputSetCooperativeLevel(false, bAlwaysProcessKeyboardInput);
				AudBlankSound();
				MemCardEject();
				MemCardCreate();
		...
		case MENU_MEMCARD_INSERT:
			if (!kNetGame && DriverSupportsMemcard()) {
				MemCardInsert();
			}
			break;
		case MENU_MEMCARD_EJECT:
			...
			break;
		case MENU_MEMCARD_PGM2_1P_SELECT:
		case MENU_MEMCARD_PGM2_2P_SELECT:
		case MENU_MEMCARD_PGM2_3P_SELECT:
		case MENU_MEMCARD_PGM2_4P_SELECT:
			if (bDrvOkay && UseDialogs() && !kNetGame && DriverUsesPgm2RawMemcard()) {
				const UINT32 slot = (id - MENU_MEMCARD_PGM2_1P_SELECT) / 3;
				InputSetCooperativeLevel(false, bAlwaysProcessKeyboardInput);
				AudBlankSound();
				MemCardSelectSlot(slot);
				GameInpCheckMouse();
			}
			break;
		case MENU_MEMCARD_PGM2_1P_CREATE:
		case MENU_MEMCARD_PGM2_2P_CREATE:
		case MENU_MEMCARD_PGM2_3P_CREATE:
		case MENU_MEMCARD_PGM2_4P_CREATE:
			if (bDrvOkay && !kNetGame && DriverUsesPgm2RawMemcard()) {
				const UINT32 slot = (id - MENU_MEMCARD_PGM2_1P_CREATE) / 3;
				InputSetCooperativeLevel(false, bAlwaysProcessKeyboardInput);
				AudBlankSound();
				MemCardCreateSlot(slot);
				GameInpCheckMouse();
			}
			break;
		case MENU_MEMCARD_PGM2_1P_EJECT:
		case MENU_MEMCARD_PGM2_2P_EJECT:
		case MENU_MEMCARD_PGM2_3P_EJECT:
		case MENU_MEMCARD_PGM2_4P_EJECT:
			if (!kNetGame && DriverUsesPgm2RawMemcard()) {
				const UINT32 slot = (id - MENU_MEMCARD_PGM2_1P_EJECT) / 3;
				MemCardEjectSlot(slot);
			}
			break;

		case MENU_MEMCARD_TOGGLE:
			if (bDrvOkay && !kNetGame && DriverSupportsMemcard()) {
				MemCardToggle();
			}
			break;
