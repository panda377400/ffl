...
{
	const TCHAR *szFileName = _T("config\\presets\\preset.ini");
	const TCHAR *szHardwareString = _T("Generic hardware");
	bool bSaveDIPs = false;
	const tIniStruct* pPreset = GameInputGetHardwarePresetDesc();

	if (pPreset) {
		szFileName = pPreset->ini;
		szHardwareString = pPreset->system;
		bSaveDIPs = pPreset->dips_in_preset;
	}
