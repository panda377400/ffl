...
TCHAR szPrevGames[SHOW_PREV_GAMES][MAX_PATH];
...
static HMENU FindMenuContainingCommand(HMENU hBaseMenu, UINT nCommand)
{
	if (hBaseMenu == NULL) {
		return NULL;
	}

	const INT32 nCount = GetMenuItemCount(hBaseMenu);
	for (INT32 i = 0; i < nCount; i++) {
		if ((UINT)GetMenuItemID(hBaseMenu, i) == nCommand) {
			return hBaseMenu;
		}

		HMENU hSubMenu = GetSubMenu(hBaseMenu, i);
		if (hSubMenu) {
			HMENU hFound = FindMenuContainingCommand(hSubMenu, nCommand);
			if (hFound) {
				return hFound;
			}
		}
...
static void EnsurePgm2MemcardMenuItems()
{
	HMENU hMemcardMenu = FindMenuContainingCommand(hMenu, MENU_MEMCARD_EJECT);
	if (hMemcardMenu == NULL) {
		return;
	}

	if (GetMenuState(hMemcardMenu, MENU_MEMCARD_PGM2_1P_SELECT, MF_BYCOMMAND) != 0xFFFFFFFF) {
		return;
	}

	static const UINT nLoadIds[4] = {
		MENU_MEMCARD_PGM2_1P_SELECT, MENU_MEMCARD_PGM2_2P_SELECT,
		MENU_MEMCARD_PGM2_3P_SELECT, MENU_MEMCARD_PGM2_4P_SELECT
	};
	static const UINT nCreateIds[4] = {
		MENU_MEMCARD_PGM2_1P_CREATE, MENU_MEMCARD_PGM2_2P_CREATE,
		MENU_MEMCARD_PGM2_3P_CREATE, MENU_MEMCARD_PGM2_4P_CREATE
	};
	static const UINT nEjectIds[4] = {
		MENU_MEMCARD_PGM2_1P_EJECT, MENU_MEMCARD_PGM2_2P_EJECT,
		MENU_MEMCARD_PGM2_3P_EJECT, MENU_MEMCARD_PGM2_4P_EJECT
	};
static const TCHAR* pszLoadLabels[4] = {
	_T("PGM2 1P 载入记忆卡..."), _T("PGM2 2P 载入记忆卡..."),
	_T("PGM2 3P 载入记忆卡..."), _T("PGM2 4P 载入记忆卡...")
};
static const TCHAR* pszEjectLabels[4] = {
	_T("PGM2 1P 弹出记忆卡"), _T("PGM2 2P 弹出记忆卡"),
	_T("PGM2 3P 弹出记忆卡"), _T("PGM2 4P 弹出记忆卡")
};

	AppendMenu(hMemcardMenu, MF_SEPARATOR, 0, NULL);

	for (INT32 i = 0; i < 4; i++) {
		TCHAR szCreateLabel[64];
		_stprintf(szCreateLabel, _T("PGM2 %dP 新建记忆卡"), i + 1);
		AppendMenu(hMemcardMenu, MF_STRING, nCreateIds[i], szCreateLabel);
		AppendMenu(hMemcardMenu, MF_STRING, nLoadIds[i], pszLoadLabels[i]);
		AppendMenu(hMemcardMenu, MF_STRING, nEjectIds[i], pszEjectLabels[i]);
	}
}
...
//	CheckMenuRadioItem(hMenu, MENU_CDIMAGE, MENU_CDIMAGE, MENU_CDIMAGE + nCDEmuSelect, MF_BYCOMMAND);
	...
	for (int i = 0; i < SHOW_PREV_GAMES; i++) {
		...
		if (_tcslen(szPrevGames[i])) {
			const TCHAR* pszExt = _tcsrchr(szPrevGames[i], _T('.'));
			DWORD dwAttrib = GetFileAttributes(szPrevGames[i]);
			if (pszExt && 0 == _tcsicmp(pszExt, _T(".dat")) && dwAttrib != INVALID_FILE_ATTRIBUTES && !(dwAttrib & FILE_ATTRIBUTE_DIRECTORY)) {
				const TCHAR* pszFullName = RomdataGetFullName(szPrevGames[i]);
				if (pszFullName && pszFullName[0]) {
					_stprintf(szText, _T("%s\t%s"), pszFullName, szPrevGames[i]);
				} else {
...
void MenuEnableItems()
{
	...
	if (bDrvOkay) {
		...
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_1P_CREATE,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_1P_SELECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_1P_EJECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_2P_CREATE,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_2P_SELECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_2P_EJECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_3P_CREATE,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_3P_SELECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_3P_EJECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_4P_CREATE,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_4P_SELECT,	MF_GRAYED | MF_BYCOMMAND);
		EnableMenuItem(hMenu, MENU_MEMCARD_PGM2_4P_EJECT,	MF_GRAYED | MF_BYCOMMAND);
		...
		if (DriverSupportsMemcard()) {
			if (!kNetGame) {
				if (DriverUsesPgm2RawMemcard()) {
					...
					static const UINT nSelectIds[4] = {
						MENU_MEMCARD_PGM2_1P_SELECT, MENU_MEMCARD_PGM2_2P_SELECT,
						MENU_MEMCARD_PGM2_3P_SELECT, MENU_MEMCARD_PGM2_4P_SELECT
					};
					static const UINT nEjectIds[4] = {
					...
					const INT32 nSlotCount = MemCardPgm2SlotCount();
					for (INT32 i = 0; i < nSlotCount && i < 4; i++) {
						EnableMenuItem(hMenu, nCreateIds[i], MF_ENABLED | MF_BYCOMMAND);
						EnableMenuItem(hMenu, nSelectIds[i], MF_ENABLED | MF_BYCOMMAND);

						if (MemCardStatusSlot(i) & 2) {
							EnableMenuItem(hMenu, nEjectIds[i], MF_ENABLED | MF_BYCOMMAND);
							bCanEject = true;
						}
						if ((MemCardStatusSlot(i) & 1) && (MemCardStatusSlot(i) & 2) == 0) {
							bCanInsert = true;
						}
					}
