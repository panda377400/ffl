...
//
// Goal:
//   Flycast renders into a Win32-owned WGL context and exposes the final color
//   texture through NaomiCoreGetHwVideoInfo(). This presenter draws that texture
//   directly into a child window of hScrnWnd, bypassing pBurnDraw, VidFrame(),
//   SoftFX, PBO and CPU readback.
//
// This is intentionally isolated from the generic video plugins. It is enabled
// only with FBNEO_AWAVE_VIDEO_MODE=direct_texture.
...
static AwaveWin32GLState g_awaveGl;
...
INT32 AwaveWin32Prepare(HWND hParent)
{
	...
	bprintf(0, _T("AWAVE Win32 GL presenter prepared: child=%p dc=%p rc=%p\n"), g_awaveGl.child, g_awaveGl.dc, g_awaveGl.rc);
	return 0;
}
...
INT32 AwaveWin32DirectPresentEnabled()
{
	...
	// Before game init, env var is enough. During emulation, ask the driver too.
	if (!g_awaveGl.rc) {
		return 1;
	}
