...
int ProcessCmdLine()
{
	...
	if (_tcslen(szName)) {
...
#if defined(_WIN32)
		char envWindowed[16];
		DWORD envWindowedLen = GetEnvironmentVariableA("FBNEO_CMD_WINDOWED", envWindowed, sizeof(envWindowed));
		if (envWindowedLen > 0 && envWindowedLen < sizeof(envWindowed) && envWindowed[0] != '0') {
			bFullscreen = 0;
			nCmdOptUsed = 2;
		}
#endif
		...
		if (_tcscmp(szOpt2, _T("-r")) == 0) {
			...
			bFullscreen = 0;
		} else if (_tcscmp(szOpt2, _T("-w")) == 0 || _tcscmp(szOpt2, _T("-m")) == 0) {
			nCmdOptUsed = 2;
			bFullscreen = 0;
		}
