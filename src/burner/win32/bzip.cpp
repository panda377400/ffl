...
static bool g_bArchiveExistenceCacheEnabled = false;
static std::set<std::basic_string<TCHAR> > g_BzipExistingArchives;
static std::set<std::basic_string<TCHAR> > g_BzipMissingArchives;

// ----------------------------------------------------------------------------

static void BzipClearArchiveExistenceCache()
{
	g_BzipExistingArchives.clear();
	g_BzipMissingArchives.clear();
}

void BzipEnableArchiveExistenceCache(bool bEnable)
{
	g_bArchiveExistenceCacheEnabled = bEnable;
	BzipClearArchiveExistenceCache();
}

static inline bool IsCrcVerificationDisabled()
{
	return (nLoadMenuShowY & DISABLECRCVERIFY) != 0;
}

static bool BzipIsMgbaHardware()
{
...
static INT32 BzipGetMgbaFallbackSubdirs(const TCHAR** subdirs, INT32 maxCount)
{
...
static bool BzipBuildMgbaFallbackPath(TCHAR* dst, const TCHAR* basePath, const TCHAR* subdir, const char* archiveName)
{
	if (dst == NULL || basePath == NULL || subdir == NULL || archiveName == NULL) {
		return false;
	}
	...
	TCHAR lowerBase[MAX_PATH] = { 0 };
	_tcsncpy(lowerBase, basePath, MAX_PATH - 1);
	_tcslwr(lowerBase);

	TCHAR lowerSubdir[32] = { 0 };
	_tcsncpy(lowerSubdir, subdir, (sizeof(lowerSubdir) / sizeof(lowerSubdir[0])) - 1);
	_tcslwr(lowerSubdir);

	TCHAR matchBackslash[40] = { 0 };
	TCHAR matchSlash[40] = { 0 };
	_stprintf(matchBackslash, _T("\\%s"), lowerSubdir);
	_stprintf(matchSlash, _T("/%s"), lowerSubdir);

	if (_tcsstr(lowerBase, matchBackslash) || _tcsstr(lowerBase, matchSlash)) {
		return false;
	}

	if ((_tcslen(basePath) + _tcslen(subdir) + strlen(archiveName)) >= (MAX_PATH - 1)) {
		return false;
	}

	_stprintf(dst, _T("%s%s%hs"), basePath, subdir, archiveName);
	return true;
}
...
static INT32 RomArchiveExists(TCHAR *szName)
{
	...
	if (g_bArchiveExistenceCacheEnabled) {
		key.assign(szName);
		if (g_BzipExistingArchives.find(key) != g_BzipExistingArchives.end()) {
			return 1;
		}
		if (g_BzipMissingArchives.find(key) != g_BzipMissingArchives.end()) {
			return 0;
		}
	}
	...
	if (g_bArchiveExistenceCacheEnabled) {
		if (ret) {
			g_BzipExistingArchives.insert(key);
		} else {
			g_BzipMissingArchives.insert(key);
		}
	}
...
static INT32 CheckRomsBoot()
{
	...
	for (INT32 i = 0; i < nRomCount; i++) {
		...
		if (IsCrcVerificationDisabled() && nState == 2) {
			nState = 1;
		}
...
INT32 BzipOpen(bool bootApp)
{
	...
	const bool bMgbaHardware = BzipIsMgbaHardware();
	const bool bHasDrvName = (pszDrvName != NULL && pszDrvName[0] != '\0');
		...
		for (INT32 d = 0; d < nAppRomPaths; d++) {	// Traverse the user-configured rom paths
			...
			if (nLoadMenuShowY & (1 << 26)) {
				for (UINT32 nCount = 0; nCount < _SubDirInfo[d].nCount; nCount++) {
					...
					if (bHasDrvName && RomArchiveExists(szDrvName)) {
						bFound = true;
			...
			if (bHasDrvName && RomArchiveExists(szDrvName)) {	// Append DrvName ROMs
				bFound = true;
			...
			if (bMgbaHardware) {
				...
				const INT32 fallbackCount = BzipGetMgbaFallbackSubdirs(fallbackSubdirs, 4);

				for (INT32 f = 0; f < fallbackCount && z < BZIP_MAX; f++) {
					TCHAR szMgbaFallback[MAX_PATH] = { 0 };
					if (BzipBuildMgbaFallbackPath(szMgbaFallback, pszAppRomPaths, fallbackSubdirs[f], szName) && RomArchiveExists(szMgbaFallback)) {
						bFound = true;

						szBzipName[z] = (TCHAR*)malloc(MAX_PATH * sizeof(TCHAR));
						_tcscpy(szBzipName[z], szMgbaFallback);
				...
				for (INT32 f = 0; bHasDrvName && f < fallbackCount && z < BZIP_MAX; f++) {
					TCHAR szDrvMgbaFallback[MAX_PATH] = { 0 };
					if (BzipBuildMgbaFallbackPath(szDrvMgbaFallback, pszAppRomPaths, fallbackSubdirs[f], pszDrvName) && RomArchiveExists(szDrvMgbaFallback)) {
						bFound = true;

						szBzipName[z] = (TCHAR*)malloc(MAX_PATH * sizeof(TCHAR));
						_tcscpy(szBzipName[z], szDrvMgbaFallback);
		...
		if (ZipOpen(TCHARToANSI(pszBzipName, NULL, 0)) == 0) {		// Open the rom zip file
			...
			for (INT32 i = 0; i < nRomCount; i++) {
				...
				if (List[nFind].nLen == ri.nLen) {
					if (!IsCrcVerificationDisabled() && ri.nCrc) {		// If we know the CRC
