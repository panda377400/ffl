...
static int FBALocaliseParseBuiltinChinese()
{
	...
	if (GetTempFileName(szTempPath, _T("fbn"), 0, szTempFile) == 0) {
		return 1;
	}

	FILE* h = _tfopen(szTempFile, _T("wb"));
	if (h == NULL) {
		DeleteFile(szTempFile);
		return 1;
	}

	fwrite(g_fbneo_builtin_zh_cn_flt, 1, g_fbneo_builtin_zh_cn_flt_len, h);
	fclose(h);

	int nRet = FBALocaliseParseFile(szTempFile);
...
int FBALocaliseInit(TCHAR* pszTemplate)
{
	...
	if (pszTemplate == NULL || _tcslen(pszTemplate) == 0) {
		nRet = FBALocaliseParseBuiltinChinese();
	} else {
	...
	if (pszTemplate && _tcslen(pszTemplate)) {
