...
static LARGE_INTEGER sRunPerfFreq = {};
static UINT64 sRunPerfTotalTicks = 0;
static UINT64 sRunPerfVidFrameTicks = 0;
static UINT64 sRunPerfPaintTicks = 0;
static UINT64 sRunPerfDwmTicks = 0;
static UINT32 sRunPerfFrameCount = 0;

static inline UINT64 runPerfNow()
{
...
static inline double runPerfToMs(UINT64 ticks)
{
	return (sRunPerfFreq.QuadPart != 0) ? ((double)ticks * 1000.0 / (double)sRunPerfFreq.QuadPart) : 0.0;
}

static void runPerfReset()
{
	if (sRunPerfFreq.QuadPart == 0) {
		QueryPerformanceFrequency(&sRunPerfFreq);
	}

	sRunPerfTotalTicks = 0;
	sRunPerfVidFrameTicks = 0;
	sRunPerfPaintTicks = 0;
	sRunPerfDwmTicks = 0;
	sRunPerfFrameCount = 0;
}

static void runPerfCommit(UINT64 totalTicks, UINT64 vidFrameTicks, UINT64 paintTicks, UINT64 dwmTicks)
{
	sRunPerfTotalTicks += totalTicks;
	sRunPerfVidFrameTicks += vidFrameTicks;
	sRunPerfPaintTicks += paintTicks;
	sRunPerfDwmTicks += dwmTicks;
	sRunPerfFrameCount++;

	if (sRunPerfFrameCount < 180) return;

	const double totalMs = runPerfToMs(sRunPerfTotalTicks) / (double)sRunPerfFrameCount;
	const double vidFrameMs = runPerfToMs(sRunPerfVidFrameTicks) / (double)sRunPerfFrameCount;
	const double paintMs = runPerfToMs(sRunPerfPaintTicks) / (double)sRunPerfFrameCount;
	const double dwmMs = runPerfToMs(sRunPerfDwmTicks) / (double)sRunPerfFrameCount;
	(void)totalMs;
	(void)vidFrameMs;
	(void)paintMs;
	(void)dwmMs;
...
#else
static void runPerfReset() {}
static void runPerfCommit(UINT64, UINT64, UINT64, UINT64) {}
#endif

static inline bool runShouldUseWindowedDwmSync()
{
	if (nVidFullscreen || !bVidDWMSync) {
		return false;
	}

	// PGM2 already runs close enough to the frame budget that waiting for an
	// extra desktop VBlank in windowed mode pushes heavy games like ddpdojt over
	// budget and produces the "half-speed / audio wobble" symptom the user sees.
	if (bDrvOkay && ((BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK) == HARDWARE_IGS_PGM2)) {
		return false;
	}
...
#if defined(FBNEO_DEBUG) && defined(_WIN32)
	UINT64 runFrameStartTicks = 0;
	UINT64 vidFrameTicks = 0;
	UINT64 paintTicks = 0;
	UINT64 dwmTicks = 0;
	if (!bPause && bDraw) {
		runFrameStartTicks = runPerfNow();
	}
#endif
...
#if defined(FBNEO_DEBUG) && defined(_WIN32)
			UINT64 dwmStartTicks = runPerfNow();
#endif
			SuperWaitVBlank();
#if defined(FBNEO_DEBUG) && defined(_WIN32)
			dwmTicks += runPerfNow() - dwmStartTicks;
#endif
		}
	}
...
#if defined(FBNEO_DEBUG) && defined(_WIN32)
				vidFrameTicks += runPerfNow() - vidFrameStartTicks;
#endif
			} else {
...
#if defined(FBNEO_DEBUG) && defined(_WIN32)
	if (!bPause && bDraw) {
		runPerfCommit(runPerfNow() - runFrameStartTicks, vidFrameTicks, paintTicks, dwmTicks);
	}
#endif
