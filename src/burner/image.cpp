...
{
	if (buffer && bufferLength >= PNG_SIG_LEN) {
		if (png_sig_check(buffer) == 0) {
			return true;
		}
	}
...
INT32 PNGLoadBuffer(IMAGE* img, unsigned char* buffer, int bufferLength, INT32 nPreset)
{
	...
	if (buffer == NULL || bufferLength <= PNG_SIG_LEN) {
		return 1;
	}

	if (png_sig_check(buffer)) {
		return 1;
	}
	...
	image_width_bytes = image_size / ihdr.height;
	...
	do {
		...
		for (int i = 0; i < (int)temp_img.width * 3; i += 3) {
			int r = temp_img.rowptr[row_info.row_num][i + 0];
			int g = temp_img.rowptr[row_info.row_num][i + 1];
			int b = temp_img.rowptr[row_info.row_num][i + 2];

			if (trans_to_black) {
				for (int j = 0; j < (int)trns.n_type3_entries; j++) {
					if (trns.type3_alpha[j] < 20 &&
						plte.entries[trns.type3_alpha[j]].red == r &&
						plte.entries[trns.type3_alpha[j]].green == g &&
						plte.entries[trns.type3_alpha[j]].blue == b)
