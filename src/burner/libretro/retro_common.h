...
#define RETRO_GAME_TYPE_GBA		17
#define RETRO_GAME_TYPE_GB		18
#define RETRO_GAME_TYPE_GBC		19
...
struct pgm2_card_slot_option
{
	int slot;
	std::string select_option_name;
	std::string select_friendly_name;
	std::string select_friendly_name_categorized;
	std::string select_info;
	std::string create_option_name;
	std::string create_friendly_name;
	std::string create_friendly_name_categorized;
	std::string create_info;
	std::string enable_option_name;
	std::string enable_friendly_name;
	std::string enable_friendly_name_categorized;
	std::string enable_info;
};

struct pgm2_card_file_option
{
	int slot;
	std::string path;
	std::string option_name;
...
static inline const char* FbneoCanonicalDrvName(const char* drvname)
{
	if (drvname == NULL) return NULL;
	if (0 == strcmp(drvname, "mgba_gba")) return "gba_gba";
	if (0 == strcmp(drvname, "mgba_gb")) return "gba_gb";
	if (0 == strcmp(drvname, "mgba_gbc")) return "gba_gbc";
	return drvname;
}

static inline const char* FbneoPublicDrvName(const char* drvname)
{
	return FbneoCanonicalDrvName(drvname);
}

static inline const char* FbneoLegacyDrvName(const char* drvname)
{
	if (drvname == NULL) return NULL;
	if (0 == strcmp(drvname, "gba_gba") || 0 == strcmp(drvname, "mgba_gba")) return "mgba_gba";
	if (0 == strcmp(drvname, "gba_gb") || 0 == strcmp(drvname, "mgba_gb")) return "mgba_gb";
	if (0 == strcmp(drvname, "gba_gbc") || 0 == strcmp(drvname, "mgba_gbc")) return "mgba_gbc";
	return NULL;
}

static inline bool FbneoDrvNamesMatch(const char* lhs, const char* rhs)
{
	if (lhs == NULL || rhs == NULL) return false;
	return strcmp(FbneoCanonicalDrvName(lhs), FbneoCanonicalDrvName(rhs)) == 0;
}
...
extern std::vector<pgm2_card_slot_option> pgm2_card_slot_options;
extern std::vector<pgm2_card_file_option> pgm2_card_file_options;
...
extern INT32 nPgm2CardSlotCount;
extern INT32 nPgm2CardMenuSlot;
...
extern INT32 CoreRomPathsLoad();
extern INT32 CoreIpsPathsLoad();
extern INT32 CoreRomDataPathsLoad();
