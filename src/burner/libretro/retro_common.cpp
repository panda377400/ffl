...
static struct retro_core_option_v2_definition var_fbneo_allow_ignore_gonggao = {
	"fbneo-gonggao",
	"Free to share, not for commercial use",
	NULL,
	"FinalBurn Neo Community Group: 797540537\nRetroArch Community Group: 556297887",
	NULL,
	"gonggao",
	{
		{ "Updated to the official version: 2026-03-06", NULL },
		{ "[Core]    Add Core for igs 017/027/m27 (Thanks to NanFangXiaoGui for helping modify the driver)", NULL },
		{ "[RomData] Compatible with Nebula format (Neo; Pgm; Cps1/2/3; Megadrive)", NULL },
		{ "[IPS]     Add quick launch support (hak; fid; lww)", NULL },
		{ "[Cheat]   Add support for compressed files (zip/7z)", NULL },
		{ "[Cheat]   Add rollback feature for main ROM cheats (by:zmhtt)", NULL },
		{ "[Core]    Add Disable CRC Checking", NULL },
		{ "[Input]   Add button macro configuration", NULL },
		{ "[Info]    Community Group: 797540537 / 556297887", NULL },
		{ "[Info]    Core Compilation: Qingtian", NULL },
	},
	"Core Compilation: Qingtian"
};
...
static struct retro_core_option_v2_definition var_fbneo_allow_ignore_crc = {
	...
	NULL,
	"Only match file names and file sizes, suitable for running hacked version of games.",
	NULL,
	NULL,
	{
		{ "disabled", NULL },
...
void set_environment()
{
	...
	var_fbneo_allow_ignore_gonggao.desc                    = RETRO_IGNORE_GONGGAO_DESC;
	var_fbneo_allow_ignore_gonggao.info                    = RETRO_IGNORE_GONGGAO_INFO;
	var_fbneo_allow_ignore_gonggao.values[ 0].value        = RETRO_GONGGAO_VALUE_0;
	var_fbneo_allow_ignore_gonggao.values[ 1].value        = RETRO_GONGGAO_VALUE_1;
	var_fbneo_allow_ignore_gonggao.values[ 2].value        = RETRO_GONGGAO_VALUE_2;
	var_fbneo_allow_ignore_gonggao.values[ 3].value        = RETRO_GONGGAO_VALUE_3;
	var_fbneo_allow_ignore_gonggao.values[ 4].value        = RETRO_GONGGAO_VALUE_4;
	var_fbneo_allow_ignore_gonggao.values[ 5].value        = RETRO_GONGGAO_VALUE_5;
	var_fbneo_allow_ignore_gonggao.values[ 6].value        = RETRO_GONGGAO_VALUE_6;
	var_fbneo_allow_ignore_gonggao.values[ 7].value        = RETRO_GONGGAO_VALUE_7;
	var_fbneo_allow_ignore_gonggao.values[ 8].value        = RETRO_GONGGAO_VALUE_8;
	var_fbneo_allow_ignore_gonggao.values[ 9].value        = RETRO_GONGGAO_VALUE_9;
	vars_systems.push_back(&var_fbneo_allow_ignore_gonggao);
	...
	int nbr_pgm2cards = bIsPgm2CardGame ? (((int)pgm2_card_slot_options.size() * 2) + (int)pgm2_card_file_options.size()) : 0;

	static const char* pgm2_card_category_keys[4] = { "memcard_p1", "memcard_p2", "memcard_p3", "memcard_p4" };
	static const char* pgm2_card_category_descs[4] = {
		RETRO_PGM2_MEMCARD_P1_CAT_DESC,
		RETRO_PGM2_MEMCARD_P2_CAT_DESC,
		RETRO_PGM2_MEMCARD_P3_CAT_DESC,
		RETRO_PGM2_MEMCARD_P4_CAT_DESC
	};
	static const char* pgm2_card_category_infos[4] = {
	...
	option_defs_us = (struct retro_core_option_v2_definition*)calloc(nbr_vars + nbr_dips + nbr_cheats + nbr_ipses + nbr_romdatas + nbr_pgm2cards + 1, sizeof(struct retro_core_option_v2_definition));
	...
	for (int slot_idx = 0; slot_idx < (int)pgm2_card_slot_options.size(); slot_idx++)
	{
		const pgm2_card_slot_option& slot_option = pgm2_card_slot_options[slot_idx];
		const char* slot_category_key = (slot_option.slot >= 0 && slot_option.slot < 4) ? pgm2_card_category_keys[slot_option.slot] : "memcard";

		option_defs_us[idx_var].key                  = slot_option.enable_option_name.c_str();
		option_defs_us[idx_var].desc                 = slot_option.enable_friendly_name.c_str();
		option_defs_us[idx_var].desc_categorized     = slot_option.enable_friendly_name_categorized.c_str();
		option_defs_us[idx_var].info                 = slot_option.enable_info.c_str();
		option_defs_us[idx_var].category_key         = slot_category_key;
		option_defs_us[idx_var].values[0].value      = "disabled";
		option_defs_us[idx_var].values[1].value      = "enabled";
		option_defs_us[idx_var].values[2].value      = NULL;
		option_defs_us[idx_var].default_value        = "disabled";
		...
		option_defs_us[idx_var].key                  = slot_option.create_option_name.c_str();
		option_defs_us[idx_var].desc                 = slot_option.create_friendly_name.c_str();
		option_defs_us[idx_var].desc_categorized     = slot_option.create_friendly_name_categorized.c_str();
		option_defs_us[idx_var].info                 = slot_option.create_info.c_str();
		option_defs_us[idx_var].category_key         = slot_category_key;
		option_defs_us[idx_var].values[0].value      = "idle";
		option_defs_us[idx_var].values[0].label      = "";
		option_defs_us[idx_var].values[1].value      = "run";
		option_defs_us[idx_var].values[1].label      = RETRO_PGM2_MEMCARD_CREATE_VALUE;
		option_defs_us[idx_var].values[2].value      = NULL;
		option_defs_us[idx_var].default_value        = "idle";
		idx_var++;

		for (int file_idx = 0; file_idx < (int)pgm2_card_file_options.size(); file_idx++)
		{
			if (pgm2_card_file_options[file_idx].slot != slot_option.slot) {
				continue;
			}

			option_defs_us[idx_var].key                  = pgm2_card_file_options[file_idx].option_name.c_str();
			option_defs_us[idx_var].desc                 = pgm2_card_file_options[file_idx].friendly_name.c_str();
			option_defs_us[idx_var].desc_categorized     = pgm2_card_file_options[file_idx].friendly_name_categorized.c_str();
			option_defs_us[idx_var].info                 = pgm2_card_file_options[file_idx].info.c_str();
			option_defs_us[idx_var].category_key         = slot_category_key;
			option_defs_us[idx_var].values[0].value      = "disabled";
			option_defs_us[idx_var].values[1].value      = "enabled";
			option_defs_us[idx_var].values[2].value      = NULL;
	...
	std::vector<struct retro_core_option_v2_category> option_cats_us;
	option_cats_us.reserve(10 + (bIsPgm2CardGame ? nPgm2CardSlotCount : 0));

	struct retro_core_option_v2_category category;

	category.key = "video";
	category.desc = RETRO_VIDEO_CAT_DESC;
	category.info = RETRO_VIDEO_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "audio";
	category.desc = RETRO_AUDIO_CAT_DESC;
	category.info = RETRO_AUDIO_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "input";
	category.desc = RETRO_INPUT_CAT_DESC;
	category.info = RETRO_INPUT_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "neogeo";
	category.desc = RETRO_NEOGEO_CAT_DESC;
	category.info = RETRO_NEOGEO_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "frameskip";
	category.desc = RETRO_FRAME_CAT_DESC;
	category.info = RETRO_FRAME_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "dipswitch";
	category.desc = RETRO_DIP_CAT_DESC;
	category.info = RETRO_DIP_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "cheat";
	category.desc = RETRO_CHEAT_CAT_DESC;
	category.info = RETRO_CHEAT_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "ips";
	category.desc = RETRO_IPS_CAT_DESC;
	category.info = RETRO_IPS_CAT_INFO;
	option_cats_us.push_back(category);

	category.key = "romdata";
	category.desc = "RomData";
	category.info = RETRO_ROMDATA_CAT_INFO;
	option_cats_us.push_back(category);

	if (bIsPgm2CardGame) {
		for (int slot = 0; slot < nPgm2CardSlotCount && slot < 4; slot++) {
			category.key = pgm2_card_category_keys[slot];
			category.desc = pgm2_card_category_descs[slot];
			category.info = pgm2_card_category_infos[slot];
			option_cats_us.push_back(category);
		}
	}
...
#ifdef FBNEO_DEBUG
	category.key = "debug";
	category.desc = RETRO_DEBUG_CAT_DESC;
	category.info = RETRO_DEBUG_CAT_INFO;
	option_cats_us.push_back(category);
#endif
