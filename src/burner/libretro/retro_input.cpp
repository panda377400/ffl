...
INT32 gPlayerFacing[MAX_PLAYERS] = {0};  // 保留旧状态位，避免影响其余路径
// hardware macros
...
static bool UseNeogeoMacroConfig()
{
	const char* drvname = BurnDrvGetTextA(DRV_NAME);
	return HW_NEOGEO || (drvname && strcmp(drvname, "kof98umh") == 0);
}

static bool UseGbMacroConfig()
{
	UINT32 hardware = BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK;
	return hardware == HARDWARE_GBA ||
		hardware == HARDWARE_GB ||
		hardware == HARDWARE_GBC;
}

static bool DisableMacroConfigForDriver()
{
	const char* drvname = BurnDrvGetTextA(DRV_NAME);
	UINT32 hardware = BurnDrvGetHardwareCode() & HARDWARE_PUBLIC_MASK;
	INT32 genre = BurnDrvGetGenreFlags();

	if (hardware == HARDWARE_IGS && (genre & (GBF_MAHJONG | GBF_CARD | GBF_CASINO))) {
		return true;
	}

	return drvname &&
		(strncmp(drvname, "lanmao", 6) == 0 ||
		 strncmp(drvname, "panda2", 6) == 0);
}

static bool ShouldUseCustomMacros()
{
	return bUseCustomMacro && !DisableMacroConfigForDriver();
}
...
#define DEFAULT_MACRO_FRAMES 4   // 默认每个步骤保持 4 帧（约 40ms）

// --------------------------------
// 配置文件路径
// --------------------------------
extern char g_system_dir[260];
...
struct MacroKey {
    MacroKeyType type;
    UINT8* ptr;
    MacroKey() : type(MK_NORMAL), ptr(NULL) {}
    MacroKey(MacroKeyType t) : type(t), ptr(NULL) {}
};
...
struct Macro {
    ...
    int facing;
    bool active;
    std::vector<UINT8*> current_keys;   // 新增：记录当前步骤已按下的按键
Macro() : player(-1), current_step(0), frame_counter(0), facing(0), active(false) {}
};

static std::map<std::string, Macro> gMacros;   // 源按钮名称 -> 宏
static std::map<std::string, UINT8*> gPlayerCharToInput[MAX_PLAYERS];
static std::set<UINT8*> gMacroKeys;
static bool gMacroDirectionLock[MAX_PLAYERS] = {false};
static std::map<std::string, int> gMacroPendingNeutralFrames;
...
static void AnalyzeGameLayout()
{
	...
	for (UINT32 i = 0; i < nGameInpCount; i++) {
		...
		// Some mahjong drivers expose keyboard inputs as "mah a" without a player prefix.
		if (strncmp("mah ", bii.szInfo, 4) == 0) {
			nMahjongKeyboards = (nMaxPlayers > 0) ? nMaxPlayers : 1;
		}
...
static bool MacroStepHasDirections(const MacroStep& step)
{
	for (size_t i = 0; i < step.keys.size(); ++i) {
		if (step.keys[i].type != MK_NORMAL) {
			return true;
		}
	}
...
static void ClearPlayerDirectionalInputs(int player)
{
	if (player < 0 || player >= MAX_PLAYERS) {
		return;
	}

	std::map<std::string, UINT8*>& key_map = gPlayerCharToInput[player];
	static const char* directions[] = { "Left", "Right", "Up", "Down" };

	for (size_t i = 0; i < sizeof(directions) / sizeof(directions[0]); ++i) {
		std::map<std::string, UINT8*>::iterator it = key_map.find(directions[i]);
...
static int GetPlayerHeldFacingDirection(unsigned port)
{
	if (port >= MAX_PLAYERS) {
		return 0;
	}

	int left = input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_LEFT);
	int right = input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_RIGHT);

	if (!left && !right) {
		int analog_x = input_cb_wrapper(port, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT, RETRO_DEVICE_ID_ANALOG_X);
		if (analog_x < -FAKE_ANALOG_DEADZONE) left = 1;
		else if (analog_x > FAKE_ANALOG_DEADZONE) right = 1;
	}
...
static int DetectFacingFromDirectionInput(unsigned port, int fallback)
{
	const int heldFacing = GetPlayerHeldFacingDirection(port);
	if (heldFacing < 0) return 1;
	if (heldFacing > 0) return 0;
	return fallback;
}

static bool TriggerMacroByPad(unsigned id, unsigned port)
{
    if (port >= MAX_PLAYERS || !ShouldUseCustomMacros()) return false;

    const char* suffix = NULL;
    if (id == RETRO_DEVICE_ID_JOYPAD_L)      suffix = "L";
    else if (id == RETRO_DEVICE_ID_JOYPAD_R) suffix = "R";
    else if (id == RETRO_DEVICE_ID_JOYPAD_L2) suffix = "L2";
    else if (id == RETRO_DEVICE_ID_JOYPAD_R2) suffix = "R2";
    else if (id == RETRO_DEVICE_ID_JOYPAD_L3) suffix = "L3";
    else if (id == RETRO_DEVICE_ID_JOYPAD_R3) suffix = "R3";
    else return false;

    char name[16];
    snprintf(name, sizeof(name), "P%d_%s", port + 1, suffix);

    std::map<std::string, Macro>::iterator it = gMacros.find(name);
    if (it != gMacros.end()) {
        Macro& m = it->second;
        if (!m.active) {
            const bool has_directions = MacroHasDirections(m);
            const bool needs_neutral = has_directions && (GetPlayerHeldFacingDirection(port) != 0);
            m.active = true;
            m.current_step = 0;
            m.frame_counter = 0;
            m.facing = DetectFacingFromDirectionInput(port, gPlayerFacing[port]);
            gPlayerFacing[port] = m.facing;
            if (needs_neutral) {
                gMacroPendingNeutralFrames[name] = DEFAULT_MACRO_FRAMES;
            } else {
                gMacroPendingNeutralFrames.erase(name);
            }
            if (has_directions) {
                gMacroDirectionLock[port] = true;
                ClearPlayerDirectionalInputs((int)port);
            }
            m.current_keys.clear();   // 清空旧按键记录
            //log_cb(RETRO_LOG_INFO, "[FBNeo] 触发宏: %s\n", name);
        }
        return true;
    }
...
static void PollPadMacroTriggers()
{
	if (!ShouldUseCustomMacros()) {
		return;
	}

	for (int port = 0; port < MAX_PLAYERS; port++) {
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_L, port);
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_R, port);
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L2))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_L2, port);
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R2))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_R2, port);
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L3))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_L3, port);
		if (input_cb_wrapper(port, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R3))
			TriggerMacroByPad(RETRO_DEVICE_ID_JOYPAD_R3, port);
	}
}

static bool IsDirectionalKeyBind(int nCode)
{
	...
	if (device == RETRO_DEVICE_ANALOG) {
		return (id == RETRO_DEVICE_ID_ANALOG_X || id == RETRO_DEVICE_ID_ANALOG_Y) &&
			(position == JOY_NEG || position == JOY_POS);
	}

	return false;
}

static void UpdateMacroDirectionLock()
{
	memset(gMacroDirectionLock, 0, sizeof(gMacroDirectionLock));

	if (!ShouldUseCustomMacros()) {
		return;
	}

	for (std::map<std::string, Macro>::iterator it = gMacros.begin(); it != gMacros.end(); ++it) {
		Macro& macro = it->second;
		if (!macro.active) continue;
		if (macro.player < 0 || macro.player >= MAX_PLAYERS) continue;

		gMacroDirectionLock[macro.player] = true;
	}
}

static inline int CinpState(int nCode)
{
	unsigned id = sKeyBinds[nCode].id;   // ✅ 必须最先定义
    int pressed = 0;

    if (sKeyBinds[nCode].index == -1) {
        pressed = input_cb_wrapper(
            sKeyBinds[nCode].port,
            sKeyBinds[nCode].device,
            0,
            id
        );
    }

    if (pressed && ShouldUseCustomMacros()) {
        if (id == RETRO_DEVICE_ID_JOYPAD_L ||
            ...
            if (TriggerMacroByPad(id, sKeyBinds[nCode].port)) {
                return 0;
            }
        }

        // Consume the stale duplicated branch below until the function is fully normalized.
        pressed = 0;
    }
	...
	if (port < MAX_PLAYERS && gMacroDirectionLock[port] && IsDirectionalKeyBind(nCode))
		return 0;
...
static INT32 GameInpOtherOne(struct GameInp* pgi, char* szi, char *szn)
{
	...
	if (nMahjongKeyboards > 0) {
		...
		if (strcmp("mah score", szi) == 0)
			GameInpDigital2RetroInpKey(pgi, mahjongKeyboardPort, RETROK_RCTRL, szn, RETRO_DEVICE_KEYBOARD);
		if (strcmp("mah big", szi) == 0)
			GameInpDigital2RetroInpKey(pgi, mahjongKeyboardPort, RETROK_RETURN, szn, RETRO_DEVICE_KEYBOARD);
		if (strcmp("mah wup", szi) == 0)
			GameInpDigital2RetroInpKey(pgi, mahjongKeyboardPort, RETROK_RSHIFT, szn, RETRO_DEVICE_KEYBOARD);
		if (strcmp("mah small", szi) == 0)
			GameInpDigital2RetroInpKey(pgi, mahjongKeyboardPort, RETROK_BACKSPACE, szn, RETRO_DEVICE_KEYBOARD);
	}
...
// 辅助函数：根据 token 返回 MacroKey 列表（支持组合方向）
static std::vector<MacroKey> parseTokenToMacroKeys(const std::string& token, int player) {
    std::vector<MacroKey> keys;
    if (player < 0 || player >= BurnDrvGetMaxPlayers()) {
        return keys;
    }
	if (token == "_") {   // 空步骤，返回空列表
        return keys;
    }
	
    if (token == "←") {
        keys.push_back(MacroKey(MK_LEFT));
    } else if (token == "→") {
        keys.push_back(MacroKey(MK_RIGHT));
    } else if (token == "↑") {
        keys.push_back(MacroKey(MK_UP));
    } else if (token == "↓") {
        keys.push_back(MacroKey(MK_DOWN));
    } else if (token == "↖") {
        keys.push_back(MacroKey(MK_LEFT));
        keys.push_back(MacroKey(MK_UP));
    } else if (token == "↗") {
        ...
        // 普通按钮：从映射表中查找真实指针
        std::map<std::string, UINT8*>::iterator it = gPlayerCharToInput[player].find(token);
        if (it != gPlayerCharToInput[player].end()) {
            MacroKey mk;
            mk.type = MK_NORMAL;
            mk.ptr = it->second;
            keys.push_back(mk);
        } else {
            if (!gPlayerCharToInput[player].empty()) {
                log_cb(RETRO_LOG_DEBUG, "[FBNeo] 玩家%d 未找到按钮 '%s'\n", player+1, token.c_str());
            }
        }
    }
    return keys;
...
// 加载宏配置文件，建立源按钮到目标按钮的映射
static void LoadMacroMappings()
{
	char path[512];

	gMacros.clear();
	gMacroPendingNeutralFrames.clear();

	if (!ShouldUseCustomMacros()) {
		log_cb(RETRO_LOG_INFO, "[FBNeo] Custom pad macros disabled for this driver, keeping original L/L2/L3/R/R2/R3 inputs\n");
		return;
	}
    ...
    // 优先使用街霸专用配置
    if (bStreetFighterLayout) {
        config_name = "input_sf.ini";
    }
    else if (bIsPgmCartGame) {
        config_name = "input_pgm.ini";
    }
    else if (bIsCps1CartGame) {
        config_name = "input_cps1.ini";
    }
    else if (UseNeogeoMacroConfig()) {
        config_name = "input_neogeo.ini";
    }
    else if (UseGbMacroConfig()) {
        config_name = "input_gb.ini";
    }
    else {
        config_name = "input_other.ini";
    }

    snprintf(path, sizeof(path), "%s/fbneo/input/%s", g_system_dir, config_name);
    log_cb(RETRO_LOG_INFO, "[FBNeo] 正在从 %s 加载宏配置\n", path);

    std::ifstream file(path);
    if(!file) {
        log_cb(RETRO_LOG_INFO, "[FBNeo] 宏配置文件 %s 不存在或无法打开\n", path);
        return;
    }

    log_cb(RETRO_LOG_INFO, "[FBNeo] 宏配置文件已应用\n");
    std::string line;
    while(std::getline(file, line)) {
        ...
        log_cb(RETRO_LOG_DEBUG, "[FBNeo] 读取到宏配置: key='%s', val='%s'\n", key.c_str(), val.c_str());

        // ===============================
        // 处理 L/R 宏（支持方向翻转）
        // ===============================
        if (key.size() >= 4 && key[0] == 'P' && key[2] == '_') {
			int p = key[1] - '0';
			if (p >= 1 && p <= MAX_PLAYERS) {
				std::string suffix = key.substr(3);
				if (suffix == "L" || suffix == "R" || suffix == "L2" || suffix == "R2" ||
					suffix == "L3" || suffix == "R3") {
					int player = p - 1;
					...
					while (iss >> token) {
						// 解析帧数
						int frames = DEFAULT_MACRO_FRAMES;
						size_t p1 = token.find('(');
						if (p1 != std::string::npos) {
							size_t p2 = token.find(')', p1);
							if (p2 != std::string::npos) {
								frames = atoi(token.substr(p1+1, p2-p1-1).c_str());
								if (frames <= 0) frames = DEFAULT_MACRO_FRAMES;
								token = token.substr(0, p1);
							}
						}

						// 处理组合键
						std::vector<std::string> keyTokens;
						std::stringstream kss(token);
						std::string kt;
						while (std::getline(kss, kt, '+')) {
							if (!kt.empty()) keyTokens.push_back(kt);
						}

						std::vector<MacroKey> stepKeys;
						for (size_t i = 0; i < keyTokens.size(); ++i) {
							std::vector<MacroKey> sub = parseTokenToMacroKeys(keyTokens[i], player);
							stepKeys.insert(stepKeys.end(), sub.begin(), sub.end());
						}

						// 关键：无论 stepKeys 是否为空，都添加步骤（空步骤用于释放按键）
						MacroStep step;
						step.keys = stepKeys;
						step.frames = frames;
					...
					if (!steps.empty()) {
						...
						m.active = false;
						gMacros[key] = m;
						//log_cb(RETRO_LOG_INFO, "[FBNeo] 注册宏触发器: %s (玩家%d)\n", key.c_str(), player+1);
					}
        ...
        UINT8* source = NULL;
		int player = -1;
		(void)player;   // 抑制未使用警告（若后续未使用）
        for (UINT32 i = 0; i < nGameInpCount; i++) {
            struct BurnInputInfo bii;
            BurnDrvGetInputInfo(&bii, i);

            const char* name = bii.szName ? bii.szName : "";
            const char* info = bii.szInfo ? bii.szInfo : "";

            int p = -1;
            if (strstr(name, "P1 ") == name || strstr(info, "P1 ") == info) p = 0;
            else if (strstr(name, "P2 ") == name || strstr(info, "P2 ") == info) p = 1;

            if (stricmp(name, key.c_str()) == 0 ||
                stricmp(info, key.c_str()) == 0) {
                source = GameInp[i].Input.pVal;
                player = p;
                break;
            }
        }
        ...
        if (!source) {
            log_cb(RETRO_LOG_INFO, "[FBNeo] 未找到源按钮: %s\n", key.c_str());
            continue;
        }
    }
}
...
INT32 GameInpDefault()
{
	...
	// 建立字符到游戏输入指针的映射（按玩家区分）
    for (int p = 0; p < MAX_PLAYERS; p++) {
        gPlayerCharToInput[p].clear();
    }

    for (UINT32 idx = 0; idx < nGameInpCount; ++idx) {
        // 注意：这里不要再声明新的 bii，直接使用外层的 bii
        BurnDrvGetInputInfo(&bii, idx);
        const char* name = bii.szName ? bii.szName : "";
        const char* info = bii.szInfo ? bii.szInfo : "";
        if (name[0] == '\0' && info[0] == '\0') continue;

        // 确定玩家编号
        int player = -1;
        if (strstr(name, "P1 ") == name || strstr(info, "P1 ") == info) player = 0;
        else if (strstr(name, "P2 ") == name || strstr(info, "P2 ") == info) player = 1;
        else if (strstr(name, "P3 ") == name || strstr(info, "P3 ") == info) player = 2;
        else if (strstr(name, "P4 ") == name || strstr(info, "P4 ") == info) player = 3;
        else continue; // 不是玩家输入，忽略

        // 尝试从 name 或 info 中提取按钮编号
        const char* button_pos = strstr(name, "Button ");
        if (!button_pos) button_pos = strstr(info, "Button ");

        int num = 0;
        const char* fire_pos = strstr(info, "fire ");
        if (!fire_pos) fire_pos = strstr(name, "fire ");
        if (fire_pos) {
            num = atoi(fire_pos + 5);
        }

        if (num <= 0 && button_pos) {
            num = atoi(button_pos + 7);
        }

        if (num <= 0) {
            if ((strstr(name, "Button A") != NULL) || (strstr(info, "Button A") != NULL)) num = 1;
            else if ((strstr(name, "Button B") != NULL) || (strstr(info, "Button B") != NULL)) num = 2;
        }

        if (num >= 1 && num <= 10) {
                char key[4];
                snprintf(key, sizeof(key), "%d", num);  // 替代 std::to_string
                gPlayerCharToInput[player][key] = GameInp[idx].Input.pVal;
                if (UseNeogeoMacroConfig() && num >= 1 && num <= 4) {
                    static const char* alias[] = { "A", "B", "C", "D" };
                    gPlayerCharToInput[player][alias[num - 1]] = GameInp[idx].Input.pVal;
                }
                log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 '%s' = %s\n", player+1, key, name ? name : info);
				if (UseNeogeoMacroConfig() && num >= 1 && num <= 4) {
					static const char* alias[] = { "A", "B", "C", "D" };
					gPlayerCharToInput[player][alias[num - 1]] = GameInp[idx].Input.pVal;
					log_cb(RETRO_LOG_INFO, "[FBNeo] 鐜╁%d 鎸夐敭 '%s' = %s\n", player+1, alias[num - 1], name ? name : info);
				}
                continue;
            }

        // 方向映射（使用字符串键）
        if (strstr(name, "Up") || strstr(info, "Up")) {
            gPlayerCharToInput[player]["Up"] = GameInp[idx].Input.pVal;
            log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 'Up' = %s\n", player+1, name ? name : info);
        } else if (strstr(name, "Down") || strstr(info, "Down")) {
            gPlayerCharToInput[player]["Down"] = GameInp[idx].Input.pVal;
            log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 'Down' = %s\n", player+1, name ? name : info);
        } else if (strstr(name, "Left") || strstr(info, "Left")) {
            gPlayerCharToInput[player]["Left"] = GameInp[idx].Input.pVal;
            log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 'Left' = %s\n", player+1, name ? name : info);
        } else if (strstr(name, "Right") || strstr(info, "Right")) {
            gPlayerCharToInput[player]["Right"] = GameInp[idx].Input.pVal;
            log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 'Right' = %s\n", player+1, name ? name : info);
        }

		// NeoGeo 按钮字母映射
		if (UseNeogeoMacroConfig()) {
			const char* btn_name = NULL;
			if (strstr(name, "Button A") != NULL || strstr(info, "Button A") != NULL) btn_name = "A";
			else if (strstr(name, "Button B") != NULL || strstr(info, "Button B") != NULL) btn_name = "B";
			else if (strstr(name, "Button C") != NULL || strstr(info, "Button C") != NULL) btn_name = "C";
			else if (strstr(name, "Button D") != NULL || strstr(info, "Button D") != NULL) btn_name = "D";

			if (btn_name) {
				gPlayerCharToInput[player][btn_name] = GameInp[idx].Input.pVal;
				log_cb(RETRO_LOG_INFO, "[FBNeo] 玩家%d 按键 '%s' = %s\n", player+1, btn_name, name ? name : info);
			}
		}
    }

	log_cb(RETRO_LOG_INFO, "[FBNeo] 即将加载宏配置...\n");
    LoadMacroMappings();
	...
	for (int i = 0; i < MAX_PLAYERS; i++)
	{
		...
		else
		{
			bool bValidDevice =
				...
				(nMahjongKeyboards > 0 && (nDeviceType[i] == RETRO_DEVICE_KEYBOARD || nDeviceType[i] == RETROMAHJONG_KEYBOARD));
			if (!bValidDevice)
			{
				nDeviceType[i] = RETROPAD_CLASSIC;
			}
			else if (nMahjongKeyboards > 0 && nDeviceType[i] == RETRO_DEVICE_KEYBOARD)
			{
				nDeviceType[i] = RETROMAHJONG_KEYBOARD;
			}
		}
	}
}
...
void SetControllerInfo()
{
	...
	if ((nHardwareCode & HARDWARE_PUBLIC_MASK) == HARDWARE_SPECTRUM || (nHardwareCode & HARDWARE_PUBLIC_MASK) == HARDWARE_MSX) {
		...
		static const struct retro_controller_description mahjong_controller_description[] = {
			{ "Classic",           RETROPAD_CLASSIC },
			{ "Modern",            RETROPAD_MODERN },
			{ "6-Button Panel",    RETROPAD_6PANEL },
			{ "Mouse (ball only)", RETROMOUSE_BALL },
			...
			{ "Lightgun",          RETRO_DEVICE_LIGHTGUN },
			{ "Analog Arcade Gun", RETROARCADE_GUN },
			{ "Keyboard",          RETROMAHJONG_KEYBOARD }
		};
		...
		{
			controller_infos[i].types = (nMahjongKeyboards > 0) ? mahjong_controller_description : controller_description;
			controller_infos[i].num_types = (nMahjongKeyboards > 0)
				? (sizeof(mahjong_controller_description) / sizeof(mahjong_controller_description[0]))
				: (sizeof(controller_description) / sizeof(controller_description[0]));
		}

		controller_infos[nMaxControllers].types = NULL;
		controller_infos[nMaxControllers].num_types = 0;
...
// 获取实际按键指针
static UINT8* getActualKeyPtr(int player, int facing, const MacroKey& key) {
    if (key.type == MK_NORMAL) return key.ptr;
    std::map<std::string, UINT8*>& map = gPlayerCharToInput[player];
    bool swapLR = (facing == 1);
    if (key.type == MK_LEFT) {
        std::map<std::string, UINT8*>::iterator it = map.find(swapLR ? "Right" : "Left");
        return (it != map.end()) ? it->second : NULL;
    }
    if (key.type == MK_RIGHT) {
        std::map<std::string, UINT8*>::iterator it = map.find(swapLR ? "Left" : "Right");
        return (it != map.end()) ? it->second : NULL;
    }
    if (key.type == MK_UP) {
        std::map<std::string, UINT8*>::iterator it = map.find("Up");
        return (it != map.end()) ? it->second : NULL;
    }
    if (key.type == MK_DOWN) {
        std::map<std::string, UINT8*>::iterator it = map.find("Down");
        return (it != map.end()) ? it->second : NULL;
    }
    return NULL;
}
...
// 更新玩家朝向（简化版，根据输入自动推断）
static void UpdatePlayerFacing() {
    for (int p = 0; p < MAX_PLAYERS; ++p) {
        if (gMacroDirectionLock[p]) {
            continue;
        }

        int heldFacing = GetPlayerHeldFacingDirection(p);
        if (heldFacing < 0) gPlayerFacing[p] = 1;
        else if (heldFacing > 0) gPlayerFacing[p] = 0;
        // 手动翻转：Select + R1
        int select = input_cb_wrapper(p, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_SELECT);
        int r1 = input_cb_wrapper(p, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R);
        static bool lastFlipState[MAX_PLAYERS] = {false};
        bool flipNow = (select && r1);
        if (flipNow && !lastFlipState[p]) {
            gPlayerFacing[p] = !gPlayerFacing[p];
        }
        lastFlipState[p] = flipNow;
    }
}
...
void InputMake(void)
{
    ...
    poll_cb();
    const bool has_sequence_macros = !gMacros.empty();
    if (has_sequence_macros) {
        UpdatePlayerFacing();
	PollPadMacroTriggers();

	// 更新玩家朝向（需根据游戏实际状态实现）
	UpdateMacroDirectionLock();
    }
    ...
    // ================= 用户自定义宏（按键序列） =================
    // 执行所有活动宏（激活由 TriggerMacroByPad 完成）
	for (std::map<std::string, Macro>::iterator it = gMacros.begin(); it != gMacros.end(); ++it) {
		Macro& macro = it->second;
		if (!macro.active) continue;

		if (macro.current_step >= (int)macro.steps.size()) {
			for (size_t k = 0; k < macro.current_keys.size(); ++k) {
				UINT8* ptr = macro.current_keys[k];
				if (ptr) *ptr = 0;
			}
			macro.current_keys.clear();
			macro.active = false;
			macro.current_step = 0;
			macro.frame_counter = 0;
			gMacroPendingNeutralFrames.erase(it->first);
			continue;
		}

		std::map<std::string, int>::iterator neutral_it = gMacroPendingNeutralFrames.find(it->first);
		if (neutral_it != gMacroPendingNeutralFrames.end()) {
			for (size_t k = 0; k < macro.current_keys.size(); ++k) {
				UINT8* ptr = macro.current_keys[k];
				if (ptr) *ptr = 0;
			}
			macro.current_keys.clear();
			ClearPlayerDirectionalInputs(macro.player);
			if (--neutral_it->second <= 0) {
				gMacroPendingNeutralFrames.erase(neutral_it);
			}
			continue;
		}

		MacroStep& step = macro.steps[macro.current_step];
		std::vector<UINT8*> previous_keys = macro.current_keys;

		// 如果是空步骤（keys为空），则清除所有宏按下的按键
		if (step.keys.empty()) {
			for (size_t k = 0; k < macro.current_keys.size(); ++k) {
				UINT8* ptr = macro.current_keys[k];
				if (ptr) *ptr = 0;
			...
			if (MacroStepHasDirections(step)) {
				ClearPlayerDirectionalInputs(macro.player);
			}
			// 普通步骤：按下当前步骤的按键，并记录到 current_keys（用于最终清除）
			for (size_t k = 0; k < step.keys.size(); ++k) {
				UINT8* ptr = getActualKeyPtr(macro.player, macro.facing, step.keys[k]);
				if (ptr) {
					*ptr = 1;
					// 避免重复记录同一指针
					bool found = false;
					for (size_t j = 0; j < macro.current_keys.size(); ++j) {
						if (macro.current_keys[j] == ptr) { found = true; break; }
					}
					if (!found) macro.current_keys.push_back(ptr);
				}
			}
		}

		if (step.keys.empty() && MacroHasDirections(macro)) {
			ClearPlayerDirectionalInputs(macro.player);
		}
		...
		for (size_t k = 0; k < step.keys.size(); ++k) {
			...
			if (!found) {
				new_keys.push_back(ptr);
			}
		}

		for (size_t k = 0; k < previous_keys.size(); ++k) {
			UINT8* old_ptr = previous_keys[k];
			bool still_needed = false;
			for (size_t j = 0; j < new_keys.size(); ++j) {
				if (new_keys[j] == old_ptr) {
					still_needed = true;
					break;
				}
			}

			if (!still_needed && old_ptr) {
				*old_ptr = 0;
			}
		}

		for (size_t k = 0; k < new_keys.size(); ++k) {
			if (new_keys[k]) {
				*new_keys[k] = 1;
			}
		}

		macro.current_keys = new_keys;
		macro.frame_counter++;
		if (macro.frame_counter >= step.frames) {
			macro.frame_counter = 0;
			macro.current_step++;
	...
	if (nBurnDrvActive != ~0U)
	{
		...
		else
		{
			bool bValidDevice =
				...
				(nMahjongKeyboards > 0 && (device == RETRO_DEVICE_KEYBOARD || device == RETROMAHJONG_KEYBOARD));
			if (!bValidDevice)
			{
				device = RETROPAD_CLASSIC;
				HandleMessage(RETRO_LOG_INFO, "[FBNeo] Unknown device type for port %d, forcing \"Classic\" instead\n", port);
			}
			else if (nMahjongKeyboards > 0 && device == RETRO_DEVICE_KEYBOARD)
			{
				device = RETROMAHJONG_KEYBOARD;
			}
		}
	}
