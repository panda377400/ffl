...
INT32 nRomOffset = 0, nActiveArray = 0, nStandalone, nDefineNum = 0;
static TCHAR szAltFile[MAX_PATH] = { 0 }, szAltName[MAX_PATH] = { 0 }, * pszAltDesc = NULL;
TCHAR ** pszIpsActivePatches = NULL;
TCHAR CoreIpsPaths[DIRS_MAX][MAX_PATH];
...
{
	INT32 nRet = 0;
	const char* pszDrvName = FbneoPublicDrvName(BurnDrvGetTextA(DRV_NAME)), * pszExt = ".dat";
	if (NULL == pszDrvName) return -2;
	const char* pszCompatDrvName = FbneoCanonicalDrvName(pszDrvName);
	const char* pszLegacyDrvName = FbneoLegacyDrvName(pszDrvName);
	bool useCompatDrvName = (0 != strcmp(pszDrvName, pszCompatDrvName));
	...
	for (INT32 i = 0; i < DIRS_MAX; i++)
	{
		...
		const char* drvNames[3] = { pszDrvName, useCompatDrvName ? pszCompatDrvName : NULL, pszLegacyDrvName };

		for (INT32 drvIdx = 0; drvIdx < 3; drvIdx++)
		{
			if (NULL == drvNames[drvIdx]) continue;
			bool duplicate = false;
			for (INT32 prev = 0; prev < drvIdx; prev++) {
				if (drvNames[prev] && 0 == strcmp(drvNames[drvIdx], drvNames[prev])) {
					duplicate = true;
					break;
				}
			}
			...
			snprintf(
				...
				CoreIpsPaths[i], PATH_DEFAULT_SLASH_C(), drvNames[drvIdx], PATH_DEFAULT_SLASH_C()
			);
	...
	if (NULL != (fp = fopen(patch_name, "rb")))
	{
		...
		while (!feof(fp))
		{
			if (NULL != fgets(s, sizeof(s), fp))
			{
                ...
                /* 修改后的匹配逻辑 - 开始 */
                bool bNameMatch = (_stricmp(rom_name, game_name) == 0);
                bool bCrcMatch = (nIps_crc == crc);
                
                if ((crc == 0 && bNameMatch) ||                     // CRC为0且名称匹配
                    (crc != 0 && bNameMatch && nIps_crc == 0) ||    // ROM有CRC但IPS未指定CRC且名称匹配
                    (crc != 0 && bCrcMatch))                        // CRC匹配
                {
                    bTarget = true;
                    /* 修改后的匹配逻辑 - 结束 */
