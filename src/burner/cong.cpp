	...
	RomDataGetStorageName(szStorageName, _countof(szStorageName));
		...
		if (NeoCDInfo_ID()) {
		  _stprintf(szName, _T("%sngcd_%s.ini"), szAppEEPROMPath, BurnDrvGetText(DRV_NAME));
			...
			_stprintf(szName, _T("%s%s.ini"), szAppEEPROMPath, szStorageName);
		}
	#else
		...
		if (NeoCDInfo_ID()) {
			...
			_stprintf(szName, _T("config/games/%s.ini"), szStorageName);
		}
	#endif
	return szName;
}
...
INT32 ConfigGameSave(bool bSave)
{
	...
	// Write title
	_ftprintf(h, _T("// ") _T(APP_TITLE) _T(" v%s --- Config File for %s (%s)\n\n"), szAppBurnVer, RomDataGetActiveDisplayName(), ANSIToTCHAR(BurnDrvGetTextA(DRV_FULLNAME), NULL, 0));
